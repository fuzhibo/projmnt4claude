...
/** 代码审核阶段角色模板 */
interface CodeReviewRoleTemplate {
  /** 角色声明 */
  roleDeclaration: string;
  /** 角色专属审核重点 */
  reviewFocus: string[];
...
const DEV_TEMPLATES: Record<RoleType, DevRoleTemplate> = {
  frontend: {
...
const CODE_REVIEW_TEMPLATES: Record<RoleType, CodeReviewRoleTemplate> = {
  frontend: {
...
const QA_TEMPLATES: Record<RoleType, QARoleTemplate> = {
  frontend: {
...
const DEFAULT_DEV: DevRoleTemplate = {
  roleDeclaration: '你是一个开发者，你的唯一职责是实现被分配任务的代码变更。',
  extraInstructions: [],
};

const DEFAULT_CODE_REVIEW: CodeReviewRoleTemplate = {
...
const DEFAULT_QA: QARoleTemplate = {
...
/** 获取开发阶段角色模板 */
export function getDevRoleTemplate(role?: string): DevRoleTemplate {
  const normalized = normalizeRole(role);
  return normalized ? DEV_TEMPLATES[normalized] : DEFAULT_DEV;
}

/** 获取代码审核阶段角色模板 */
export function getCodeReviewRoleTemplate(role?: string): CodeReviewRoleTemplate {
  const normalized = normalizeRole(role);
  return normalized ? CODE_REVIEW_TEMPLATES[normalized] : DEFAULT_CODE_REVIEW;
}

/** 获取 QA 阶段角色模板 */
export function getQARoleTemplate(role?: string): QARoleTemplate {
  const normalized = normalizeRole(role);
