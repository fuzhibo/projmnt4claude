 ...
 * 获取证据目录路径
 */
export function getEvidenceDir(taskId: string, cwd: string = process.cwd()): string {
 ...
 * 确保证据目录存在
 */
export function ensureEvidenceDir(taskId: string, cwd: string = process.cwd()): string {
...
): Promise<{ exitCode: number; output: string; evidencePath: string }> {
...
export async function validateTaskCompletion(
  ...
  const incompleteCheckpoints = checkpoints.filter(
    (cp: CheckpointMetadata) => cp.status !== 'completed' && cp.status !== 'skipped'
  );
  ...
  if (incompleteCheckpoints.length > 0) {
    errors.push({
      ...
      details: incompleteCheckpoints.map((cp: CheckpointMetadata) => `${cp.id}: ${cp.description}`),
    });
  }

  // 3. 禁止 manual 验证
  const manualCheckpoints = checkpoints.filter(
    (cp: CheckpointMetadata) => (cp.verification?.method as string) === 'manual'
  );
  ...
  if (manualCheckpoints.length > 0) {
    errors.push({
      ...
      details: manualCheckpoints.map(
        (cp: CheckpointMetadata) =>
          `${cp.id}: 请使用 code_review/lint/functional_test/e2e_test/architect_review/automated 等具体验证方法`
      ),
    });
  }
  ...
  if (executeCommands && collectEvidence) {
    for (const cp of checkpoints) {
      if (cp.status !== 'completed' || !cp.verification?.commands) continue;

      // 跳过 manual 验证（已在上面报错）
      if ((cp.verification?.method as string) === 'manual') continue;

      try {
        const evidence = await executeVerificationCommands(
          taskId,
          cp.id,
          cp.verification.commands,
          cwd
        );
  ...
  // 5. 检查验证证据
  const missingEvidence = checkpoints.filter(
    (cp: CheckpointMetadata) =>
      cp.status === 'completed' &&
      cp.verification &&
  ...
  if (missingEvidence.length > 0) {
    warnings.push({
      ...
      details: missingEvidence.map((cp: CheckpointMetadata) => cp.id),
    });
  }
 ...
 * 批量验证多个任务
 */
export async function validateMultipleTasks(
  taskIds: string[],
  cwd: string = process.cwd(),
  options?: {
    executeCommands?: boolean;
...
): Promise<Map<string, TaskValidationResult>> {
 ...
 */
export function hasManualVerification(task: TaskMeta): boolean {
  const checkpoints = task.checkpoints || [];
 ...
 * 获取所有使用 manual 验证的任务
 */
export function getTasksWithManualVerification(
  cwd: string = process.cwd()
): TaskMeta[] {
  const tasks = getAllTasks(cwd, false);
