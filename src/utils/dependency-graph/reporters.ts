 ...
 * - 异常摘要报告 (analyze 命令)
 */
import type { NodeId, GraphAnomaly } from './types.js';
 ...
 * 用于 status 命令
 */
export function renderGraphOverview(graph: DependencyGraph): string {
 ...
 */
export function renderDevelopmentFocus(graph: DependencyGraph): string {
 ...
 * 列出所有连接多个任务树的关键节点
 */
export function renderBridgeReport(
  bridges: Array<{ nodeId: NodeId; bridgedRoots: NodeId[] }>,
): string {
  if (bridges.length === 0) {
    return '未检测到桥接节点';
  }
  ...
  for (const bridge of bridges) {
    lines.push(
      `├── ${bridge.nodeId} — 连接 ${bridge.bridgedRoots.length} 棵任务树: ${bridge.bridgedRoots.join(', ')}`,
    );
  }
 ...
 * 用于 analyze 命令
 */
export function renderAnomalySummary(anomalies: GraphAnomaly[]): string {
