 ...
 * DependencyGraph 核心类
 *
 * 统一任务依赖关系的 DAG 抽象，提供图构建、查询、分析和统计功能。
...
const DEFAULT_EDGE_META: EdgeMeta = {
  source: 'explicit',
  confidence: 1.0,
};
...
export class DependencyGraph {
  ...
  private adjacency: Map<NodeId, Map<NodeId, EdgeMeta>>;

  /** 反向邻接表: node → 依赖它的节点集合 */
  private reverseAdj: Map<NodeId, Set<NodeId>>;

  /** 传递闭包（懒计算） */
  private _transitiveClosure?: Map<NodeId, Set<NodeId>>;
   ...
   * 从任务列表构建完整依赖图
   */
  static fromTasks(tasks: TaskMeta[], options?: GraphBuildOptions): DependencyGraph {
    ...
    for (const task of tasks) {
      if (excludeTerminal && terminalStatuses.has(task.status)) continue;
      graph.addNode(task);
    }
   ...
   * 增量添加节点（任务创建时调用）
   */
  addNode(task: TaskMeta, inferredDeps?: Array<{ depId: NodeId; meta: EdgeMeta }>): void {
   ...
   */
  addEdge(from: NodeId, to: NodeId, meta?: EdgeMeta): boolean {
   ...
   * 获取直接上游（当前任务依赖的任务）
   */
  getDirectUpstream(taskId: NodeId): NodeId[] {
   ...
   * 获取直接下游（依赖当前任务的任务）
   */
  getDirectDownstream(taskId: NodeId): NodeId[] {
    const downstream = this.reverseAdj.get(taskId);
   ...
   * 获取所有上游（传递闭包）
   */
  getAllUpstream(taskId: NodeId): NodeId[] {
   ...
   * 获取所有下游（传递闭包）
   */
  getAllDownstream(taskId: NodeId): NodeId[] {
   ...
   * 获取边的元数据
   */
  getEdgeMeta(from: NodeId, to: NodeId): EdgeMeta | undefined {
   ...
   * 获取节点信息
   */
  getNode(taskId: NodeId): GraphNode | undefined {
   ...
   * 拓扑排序（分层）
   */
  topologicalSort(): TopologicalOrderResult {
  ...
  findComponents(): ComponentInfo[] {
    ...
    const bridgeMap = new Map<NodeId, NodeId[]>();
  ...
  findBridgeNodes(): Array<{
    nodeId: NodeId;
    bridgedRoots: NodeId[];
    pathsFromRoots: Map<NodeId, NodeId[]>;
  }> {
    const roots = new Set(this.getRoots());
    const bridges = detectBridgeNodes(this.reverseAdj, roots);

    return bridges.map(b => {
      const pathsFromRoots = new Map<NodeId, NodeId[]>();
   ...
   * 计算失败级联影响范围
   */
  computeCascadeImpact(failedTaskId: NodeId): CascadeResult {
    ...
    const visited = new Map<NodeId, { depth: number; path: NodeId[]; source: NodeId }>();
    const queue: Array<[NodeId, number, NodeId[], NodeId]> = [];
    const downstreamSet = new Set(allDownstream);
    downstreamSet.add(failedTaskId);

    for (const depId of directAffected) {
      const entry: [NodeId, number, NodeId[], NodeId] = [depId, 1, [failedTaskId, depId], depId];
      queue.push(entry);
  ...
  getStatistics(): GraphStatistics {
    ...
    const anomalySummary: Record<GraphAnomalyType, number> = {
      cycle: 0,
      orphan: 0,
      bridge: 0,
      orphan_suspected: 0,
   ...
   * 生成主开发方向分析
   */
  getPrimaryDevelopmentFocus(): Array<{
    componentId: NodeId;
    rootNode: NodeId;
    size: number;
    description: string;
  ...
  private addEdgeInternal(from: NodeId, to: NodeId, meta: EdgeMeta): void {
  ...
  private invalidateClosure(): void {
    this._transitiveClosure = undefined;
  }

  private getTransitiveClosure(): Map<NodeId, Set<NodeId>> {
  ...
  private toSimpleAdjacency(): Map<NodeId, Set<NodeId>> {
  ...
  private findPath(from: NodeId, to: NodeId): NodeId[] {
    ...
    const queue: Array<[NodeId, NodeId[]]> = [[from, [from]]];
  ...
  private findPathThroughDep(intermediate: NodeId, target: NodeId): NodeId[] | null {
    // Check if target is reachable from intermediate via adjacency
    const visited = new Set<NodeId>();
    const queue: NodeId[] = [intermediate];
    ...
    while (queue.length > 0) {
      ...
      if (deps) {
        for (const depId of deps.keys()) {
          if (depId === target) return [intermediate, target];
  ...
  private isReachable(from: NodeId, to: NodeId): boolean {
    const visited = new Set<NodeId>([from]);
