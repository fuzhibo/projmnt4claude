 ...
 * 提供依赖关系质量门禁验证:
 * - 孤立任务合理性验证
 * - 新任务依赖完整性验证 (GATE-DEP-001/002/003)
 * - 计划操作依赖完整性验证
 */
 ...
 */
export function validateOrphan(
  nodeId: NodeId,
  graph: DependencyGraph,
  allTasks: TaskMeta[],
): { isLikelyOrphan: boolean; reason: string; relatedTasks?: NodeId[] } {
  const node = graph.getNode(nodeId);
  if (!node) {
    return { isLikelyOrphan: true, reason: '节点不存在于图中' };
  }

  const relatedTasks: NodeId[] = [];
 ...
 */
export function validateNewTaskDeps(
  taskId: NodeId,
  dependencies: NodeId[],
  graph: DependencyGraph,
  _allTasks: TaskMeta[],
  ...
  if (dependencies.length === 0) {
    ...
    warnings.push(
      `GATE-DEP-001: 任务 ${taskId} 无任何依赖。如果这不是独立模块，请添加相关依赖`,
    );
  }
  ...
  for (const depId of dependencies) {
    if (graph.hasNode(taskId) && graph.hasNode(depId)) {
      if (graph.wouldCreateCycle(taskId, depId)) {
        errors.push(
          `GATE-DEP-002: 添加 ${taskId} → ${depId} 依赖会形成循环依赖`,
        );
      }
    }
  }
  ...
  if (dependencies.length > 0) {
    ...
    if (inferredOnly.length === dependencies.length && dependencies.length > 0) {
      warnings.push(
        `GATE-DEP-003: 任务 ${taskId} 的所有依赖均为推断来源，建议确认是否需要显式声明`,
      );
    }
  }
...
export function validatePlanOperation(
  operation: 'delete' | 'reprioritize' | 'merge' | 'split',
  targetIds: NodeId[],
  graph: DependencyGraph,
  ...
  switch (operation) {
    case 'delete': {
      for (const id of targetIds) {
        ...
        if (downstream.length > 0) {
          warnings.push(
            `删除 ${id} 会影响 ${downstream.length} 个下游任务: ${downstream.join(', ')}`,
          );
        }
        ...
        if (transitiveDownstream.length > 0) {
          warnings.push(
            `删除 ${id} 的传递影响范围还包括: ${transitiveDownstream.join(', ')}`,
          );
        }
      }
      break;
    ...
    case 'merge': {
      ...
      for (const id of targetIds) {
        ...
        if (missing.length > 0) {
          warnings.push(
            `${id} 缺少其他合并目标的外部依赖: ${missing.join(', ')}，合并后需确认`,
          );
        }
      }
      break;
