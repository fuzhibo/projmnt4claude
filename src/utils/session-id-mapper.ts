 ...
 * - active: 重试时携带 --resume 续接既有会话完整历史
 *
 * 状态由 probeSessionState() 探测既有映射决定，
 * buildSessionCliArgs() 根据状态构造 CLI 参数分支。
 */
export type SessionState = 'fresh' | 'active';
...
export class SessionIdMapper {
  ...
  deriveDeterministicUuid(
    ...
    const hash = createHash('sha256')
      .update(`${internalId}|${taskId}|${phase}|${runSalt}`)
      .digest('hex');
