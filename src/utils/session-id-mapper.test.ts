...
describe('SessionIdMapper', () => {
  ...
  describe('deterministic derivation (CP-01)', () => {
    it('should produce identical UUID for identical inputs', () => {
      ...
      const uuidA = mapperA.generate(internalId, taskId, phase);
      const uuidB = mapperB.generate(internalId, taskId, phase);
    ...
    it('should be idempotent: repeated generate() returns same UUID', () => {
      ...
      const first = mapper.generate(internalId, 'TASK-100', 'codeReview');
      const second = mapper.generate(internalId, 'TASK-100', 'codeReview');
    ...
    it('should differ when taskId changes', () => {
      ...
      const uuidA = mapper.generate(internalId, 'TASK-A', 'development');
      const uuidB = mapper.generate(internalId, 'TASK-B', 'development');
    ...
    it('should differ when phase changes', () => {
      ...
      const devUuid = mapper.generate(internalId, 'TASK-200', 'development');
      const crUuid = mapper.generate(internalId, 'TASK-200', 'codeReview');
    ...
    it('should clean up old cliUuid mapping when internalId reused with new phase', () => {
      ...
      const devUuid = mapper.generate(internalId, 'TASK-300', 'development');
      // internalId 现在切换到 codeReview 阶段
      const crUuid = mapper.generate(internalId, 'TASK-300', 'codeReview');
    ...
    it('should always produce valid UUID v4 format with correct version and variant', () => {
      ...
      // 测试多组输入，确保格式恒定
      for (let i = 0; i < 50; i++) {
        const uuid = mapper.generate(`id-${i}`, `TASK-${i}`, 'development');
  ...
  describe('deserialize (CP-06)', () => {
    it('should rebuild mappings from serialize() output', () => {
      ...
      source.generate('dev-TASK-1', 'TASK-1', 'development');
      source.generate('cr-TASK-2', 'TASK-2', 'codeReview');
    ...
    it('should preserve bidirectional lookup after deserialize', () => {
      ...
      const cliUuid = source.generate(internalId, 'TASK-400', 'qaVerification');
      const serialized = source.serialize();
    ...
    it('should support serialize → deserialize round trip with deterministic UUID', () => {
      ...
      original.generate(internalId, 'TASK-500', 'evaluation');

      const restored = new SessionIdMapper();
      restored.deserialize(original.serialize());

      // 派生 UUID 与原始一致（确定性）
      const derivedAgain = restored.generate(internalId, 'TASK-500', 'evaluation');
  ...
  describe('audit logger (CP-05)', () => {
    it('should invoke audit logger on generate()', () => {
      ...
      mapper.generate('dev-TASK-600', 'TASK-600', 'development');
    ...
    it('should mark reused entries as isReused=true', () => {
      ...
      mapper.generate('cr-TASK-700', 'TASK-700', 'codeReview');
      mapper.generate('cr-TASK-700', 'TASK-700', 'codeReview'); // 幂等重用
    ...
    it('should not invoke logger when not set', () => {
      const mapper = new SessionIdMapper();
      expect(() => mapper.generate('safe-TASK', 'TASK', 'evaluation')).not.toThrow();
    });
  });
