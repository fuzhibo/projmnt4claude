...
export class APIErrorAnalyzer implements LogAnalyzer {
  readonly name = 'api-error-analyzer';
  readonly category: AnalysisCategory = 'api-error';
  ...
  async analyze(
    ...
    // 规则 1: 检测 429 速率限制
    const rateLimits = apiEntries.filter(e =>
      e.message.includes('429') || e.message.includes('rate limit') || e.message.includes('Rate limit')
    );
    ...
    // 规则 2: 检测 500 服务端错误
    const serverErrors = apiEntries.filter(e =>
      e.message.includes('500') || e.message.includes('502') || e.message.includes('503')
    );
    ...
    // 规则 3: 检测权限错误
    const authErrors = apiEntries.filter(e =>
      e.message.includes('permission') || e.message.includes('auth') ||
    ...
    const tokenEntries = entries.filter(e =>
      e.data && typeof e.data === 'object' && ('totalTokens' in e.data || 'tokensUsed' in e.data)
    );
