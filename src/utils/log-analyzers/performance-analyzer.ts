...
export class PerformanceAnalyzer implements LogAnalyzer {
  ...
  /** 慢操作阈值(ms) */
  private readonly SLOW_THRESHOLD_MS = 60_000; // 1 分钟
  /** 极慢操作阈值(ms) */
  private readonly VERY_SLOW_THRESHOLD_MS = 300_000; // 5 分钟
  ...
  async analyze(
    ...
    const perfEntries = entries.filter(e =>
      e.data && typeof e.data === 'object' && 'durationMs' in e.data
    );

    // 同时筛选超时相关日志
    const timeoutEntries = entries.filter(e =>
      e.message.includes('超时') || e.message.includes('timeout') || e.message.includes('timed out')
    );

    // 规则 1: 检测慢操作
    const slowOps = perfEntries.filter(e => {
      const duration = Number((e.data as Record<string, unknown>)?.durationMs || 0);
