...
export class HeadlessClaudeAnalyzer implements LogAnalyzer {
  readonly name = 'headless-claude-analyzer';
  readonly category: AnalysisCategory = 'headless-claude';
  readonly supportedStrategies: AnalysisStrategy[] = ['rule', 'hybrid'];
  ...
  async analyze(
    ...
    // 规则 1: 检测连续调用失败
    const failedInvokes = aiEntries.filter(e =>
      e.level === 'error' &&
    ...
    const jsonErrors = aiEntries.filter(e =>
      e.level === 'error' &&
    ...
    // 规则 3: 检测超时
    const timeouts = aiEntries.filter(e =>
      e.message.includes('超时') || e.message.includes('timeout') || e.message.includes('ETIMEDOUT')
    );
    ...
    // 规则 4: 检测重试循环
    const retryEntries = aiEntries.filter(e =>
      e.message.includes('重试') || e.message.includes('retry') || e.message.includes('Retrying')
    );
