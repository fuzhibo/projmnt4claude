...
import {
  ...
  detectBridgeNodes,
  computeDepths,
} from '../utils/dependency-graph/algorithms.js';
import type { EdgeMeta, GraphNode, NodeId } from '../utils/dependency-graph/types.js';

// ============== 辅助函数 ==============

/** 构建简单邻接表 */
function buildAdjacency(edges: [string, string][]): Map<NodeId, Set<NodeId>> {
...
function buildWeightedAdjacency(
  edges: Array<{ from: string; to: string; meta?: Partial<EdgeMeta> }>,
): Map<NodeId, Map<NodeId, EdgeMeta>> {
  const adj = new Map<NodeId, Map<NodeId, EdgeMeta>>();
...
/** 构建节点映射 */
function buildNodes(ids: string[]): Map<NodeId, GraphNode> {
...
/** 构建反向邻接表 */
function buildReverseAdj(edges: [string, string][]): Map<NodeId, Set<NodeId>> {
...
describe('wouldCreateCycleBFS', () => {
  ...
  test('添加边形成环返回 true', () => {
    ...
    // 添加 B→A 会形成环 B→A→B
    const revAdj = new Map<NodeId, Set<NodeId>>();
