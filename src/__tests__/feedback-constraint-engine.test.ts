...
describe('FeedbackConstraintEngineImpl.validate', () => {
  ...
  test('fills severity from rule when violation lacks it', () => {
    ...
    const rule: ValidationRule = {
      ...
      description: 'test',
      severity: 'warning',
      check: () => ({ ruleId: 'no-sev', severity: undefined as any, message: 'm' }),
    };
...
function mockInvoke(responses: Array<{ output: string; exitCode: number }>) {
  let i = 0;
...
describe('FeedbackConstraintEngineImpl misc', () => {
  test('setTemplate replaces template', () => {
    ...
    const feedback = engine.buildFeedback(
      [{ ruleId: 'r', severity: 'error', message: 'm' }],
      'output',
    );
