/**
 * EvalReportExistsChecker 单元测试
 *
 * 测试评估报告存在性检查器 (R-EVAL-POST-001):
 * - evaluation-report.json 存在时通过
 * - evaluation-report.json 不存在时失败
 * - 路径解析正确性
 * - 缺少输出目录时正确报错
 * - 报告内容无关性
 */
...
import type { PostEvalCheckContext } from '../../../utils/post-eval-gate/types.js';
...
describe('EvalReportExistsChecker', () => {
  const checker = new EvalReportExistsChecker();
  let testDir: string;

  beforeEach(() => {
    testDir = fs.mkdtempSync('/tmp/eval-report-existence-test-');
  });
  ...
  // 辅助函数: 创建 evaluation-report.json 文件
  function createEvalReport(taskId: string, content: object = { version: '1.0.0', taskId, result: 'PASS', evaluatedAt: new Date().toISOString(), evaluator: 'test', summary: 'test' }): void {
    const outputsDir = path.join(testDir, '.projmnt4claude', 'outputs', taskId);
    fs.mkdirSync(outputsDir, { recursive: true });
    fs.writeFileSync(path.join(outputsDir, 'evaluation-report.json'), JSON.stringify(content, null, 2));
  }
  ...
  describe('R-EVAL-POST-001: 评估报告存在性检查', () => {
    it('evaluation-report.json 存在时应通过', async () => {
      const taskId = 'TASK-exist-001';
      createEvalReport(taskId);
      ...
      expect(result.ruleId).toBe('R-EVAL-POST-001');
      expect(result.passed).toBe(true);
      expect(result.severity).toBe('ERROR');
      expect(result.message).toContain('评估报告存在');
      expect(result.details?.exists).toBe(true);
    });

    it('evaluation-report.json 不存在时应失败', async () => {
  ...
  describe('路径解析', () => {
    it('应正确解析 .projmnt4claude/outputs/{taskId}/evaluation-report.json 路径', async () => {
      ...
      expect(result.details?.reportPath).toBe(
        path.join(testDir, '.projmnt4claude', 'outputs', taskId, 'evaluation-report.json')
      );
      expect(result.details?.reportName).toBe('evaluation-report.json');
    });

    it('不同任务ID应解析不同路径', async () => {
      const taskId1 = 'TASK-aaa-001';
      const taskId2 = 'TASK-bbb-002';
      createEvalReport(taskId1);
      // taskId2 不创建报告
    ...
    it('不同工作目录应影响路径解析', async () => {
      ...
      fs.writeFileSync(
        path.join(dir1, '.projmnt4claude', 'outputs', taskId, 'evaluation-report.json'),
        '{}'
      );
  ...
  describe('报告内容无关性', () => {
    ...
    it('无效JSON内容的报告文件仍应通过存在性检查', async () => {
      ...
      const outputsDir = path.join(testDir, '.projmnt4claude', 'outputs', taskId);
      fs.mkdirSync(outputsDir, { recursive: true });
      fs.writeFileSync(path.join(outputsDir, 'evaluation-report.json'), 'not valid json');
