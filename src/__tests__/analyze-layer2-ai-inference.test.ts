...
import {
  ...
  runAIStatusInference,
  detectStatusInferenceIssues,
...
describe('detectStatusInferenceIssues', () => {
  ...
  test('full e2e: Layer 1 clean + Layer 2 AI detects stale in_progress (CP-26)', async () => {
    ...
    const layer1Issues = issues.filter(i =>
      ['report_status_mismatch', 'checkpoint_status_mismatch', 'missing_pipeline_evidence'].includes(i.type)
    );
