...
import type { ArchitectureLayer } from '../utils/ai-metadata';
 ...
 * 收集所有批次中的唯一任务ID
 */
function collectUniqueTaskIds(batches: ExecutionBatch[]): Set<string> {
...
describe('场景A (CP-13/CP-19): 文件重叠任务被识别为依赖', () => {
  ...
  test('文件重叠的任务应出现在同一条链中', () => {
    ...
    const mergedChain = chains.find(c =>
      c.tasks.some(t => t.id === 'TASK-A') && c.tasks.some(t => t.id === 'TASK-B')
    );
...
describe('场景B (CP-14/CP-20): 创建时 dependencies 被正确写入', () => {
  ...
  test('Layer2 文件重叠依赖写入 meta.json 的 dependencies 字段格式', () => {
    const task: TaskMeta = createMockTask({
...
describe('CP-4: 验证创建时 dependencies 被正确写入', () => {
  ...
  test('推断依赖的 source 字段正确区分层级', () => {
    const fileOverlap: InferredDependency = {
      depTaskId: 'TASK-A',
      overlappingFiles: ['src/utils/helper.ts'],
      source: 'file-overlap',
    };
    const aiSemantic: InferredDependency = {
      depTaskId: 'TASK-B',
      overlappingFiles: [],
      source: 'ai-semantic',
