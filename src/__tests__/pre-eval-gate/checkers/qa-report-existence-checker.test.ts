/**
 * QAReportExistenceChecker 单元测试
 *
 * 测试QA报告存在性检查器 (R-EVAL-PRE-004):
 * - qa-report.json 存在时通过
 * - qa-report.json 不存在时失败
 * - 路径解析正确性
 * - 缺少阶段报告时正确报错
 */
...
describe('QAReportExistenceChecker', () => {
  const checker = new QAReportExistenceChecker();
  let testDir: string;

  beforeEach(() => {
    testDir = fs.mkdtempSync('/tmp/qa-report-existence-test-');
  });
  ...
  // 辅助函数: 创建 qa-report.json 文件
  function createQAReport(taskId: string, content: object = { version: '1.0.0', taskId, verdict: 'PASS' }): void {
    const outputsDir = path.join(testDir, '.projmnt4claude', 'outputs', taskId);
    fs.mkdirSync(outputsDir, { recursive: true });
    fs.writeFileSync(path.join(outputsDir, 'qa-report.json'), JSON.stringify(content, null, 2));
  }
  ...
  describe('R-EVAL-PRE-004: QA报告存在性检查', () => {
    it('qa-report.json 存在时应通过', async () => {
      ...
      expect(result.ruleId).toBe('R-EVAL-PRE-004');
      expect(result.passed).toBe(true);
      expect(result.severity).toBe('ERROR');
      expect(result.message).toContain('qa-report.json 存在');
      expect(result.details?.exists).toBe(true);
    });

    it('qa-report.json 不存在时应失败', async () => {
      ...
      expect(result.severity).toBe('ERROR');
      expect(result.message).toContain('qa-report.json 不存在');
      expect(result.message).toContain('QA阶段未生成报告');
      expect(result.details?.exists).toBe(false);
    });
  });

  describe('路径解析', () => {
    it('应正确解析 .projmnt4claude/outputs/{taskId}/qa-report.json 路径', async () => {
      ...
      expect(result.details?.reportPath).toBe(
        path.join(testDir, '.projmnt4claude', 'outputs', taskId, 'qa-report.json')
      );
      expect(result.details?.reportName).toBe('qa-report.json');
    });

    it('不同任务ID应解析不同路径', async () => {
      const taskId1 = 'TASK-aaa-001';
      const taskId2 = 'TASK-bbb-002';
      createQAReport(taskId1);
      // taskId2 不创建报告
    ...
    it('不同工作目录应影响路径解析', async () => {
      ...
      fs.writeFileSync(
        path.join(dir1, '.projmnt4claude', 'outputs', taskId, 'qa-report.json'),
        '{}'
      );
  ...
  describe('报告内容无关性', () => {
    ...
    it('无效JSON内容的报告文件仍应通过存在性检查', async () => {
      ...
      const outputsDir = path.join(testDir, '.projmnt4claude', 'outputs', taskId);
      fs.mkdirSync(outputsDir, { recursive: true });
      fs.writeFileSync(path.join(outputsDir, 'qa-report.json'), 'not valid json');
