...
启用后，每个批次完成时会自动执行 `git add -A` + `git commit` + `git tag`，commit message 包含批次标签和统计信息（通过/失败/文件变更数），同时创建 git tag 标记批次完成。配合 `--dry-run` 可预览提交行为。
...
6. **批次提交**: 启用 `--batch-git-tag-commit` 后，每个批次完成时自动 git commit + git tag。commit message 格式：`harness: 批次 N 完成 (X 通过, Y 失败, Z 文件变更)`，tag 格式：`batch-{N}-{timestamp}`。`--continue` 恢复时不会重复提交已完成批次的变更
