...
import { getProjectDir, ensureDir, isInitialized, getConfigPath, getTasksDir, getArchiveDir, getToolboxDir, getBinDir, getReportsDir, getLogsDir, } from '../utils/path';
 ...
 * 初始化项目管理环境
 */
export async function setup(cwd = process.cwd(), options = {}) {
    ...
    if (gitHookConfig.enabled) {
        ...
        if (fs.existsSync(pre.gitDir)) {
            try {
                ...
                console.log(`  ⚠️ Git Hook 创建失败: ${e.message}`);
            }
        }
    }
    else {
 ...
 * 复制技能文件到项目
 */
function copySkillFiles(cwd, language, t) {
    console.log(`\n📦 ${t.copyingSkills}`);
    // 获取插件根目录
