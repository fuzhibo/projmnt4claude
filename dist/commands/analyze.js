...
import { readTaskMeta, getAllTasks, getAllTaskIds, taskExists, isSubtask, buildTaskVerification, renameTask, } from '../utils/task';
import { parseTaskId, generateTaskId, inferTaskType, CURRENT_TASK_SCHEMA_VERSION, PIPELINE_INTERMEDIATE_STATUSES, PIPELINE_STATUS_MIGRATION_MAP, normalizeStatus, normalizePriority, } from '../types/task';
...
import { inferCheckpointsFromDescription, generateStructuredDescription } from '../utils/description-template';
...
import { AIMetadataAssistant, sortFilesByLayer, LAYER_DEFINITIONS } from '../utils/ai-metadata';
import { withAIEnhancement } from '../utils/ai-helpers';
import { invokeAgent } from '../utils/headless-agent';
import { parseCheckRange, getTasksByRange, AnalyzeError } from '../utils/analyze-range-parser';
import { extractFilePaths, evaluateRelatedFiles, evaluateDescription, evaluateCheckpoints, evaluateSolution, validateCheckpoints, calculateContentQuality, checkQualityGate, validateFilesExist, DEFAULT_QUALITY_GATE_CONFIG, } from '../utils/quality-gate';
import { inferDependenciesBatch } from '../utils/dependency-engine';
import { DependencyGraph, renderAnomalySummary, renderBridgeReport, } from '../utils/dependency-graph';
const DEFAULT_ANALYZE_CONFIG = {
 ...
 * 读取 analyze 配置，合并默认值
 */
export function readAnalyzeConfig(cwd = process.cwd()) {
    const config = readConfig(cwd);
    if (!config)
        return { ...DEFAULT_ANALYZE_CONFIG };
    const analyzeRaw = config.analyze;
    if (!analyzeRaw || typeof analyzeRaw !== 'object')
        return { ...DEFAULT_ANALYZE_CONFIG };
    const userConfig = analyzeRaw;
    return {
        ...
        checkpointGenerator: ['rule-based', 'ai-powered', 'hybrid'].includes(userConfig.checkpointGenerator)
        ...
        ignorePatterns: Array.isArray(userConfig.ignorePatterns) &&
            userConfig.ignorePatterns.every((p) => typeof p === 'string')
 ...
 */
export function matchesIgnorePattern(taskId, patterns) {
 ...
 * 有效的任务状态值
 */
const VALID_STATUSES = ['open', 'in_progress', 'wait_review', 'wait_qa', 'wait_evaluation', 'resolved', 'closed', 'abandoned', 'failed'];
/**
 * 有效的任务类型
 */
 ...
 * 有效的优先级
 */
const VALID_PRIORITIES = ['P0', 'P1', 'P2', 'P3', 'Q1', 'Q2', 'Q3', 'Q4'];
/**
 * 获取从 fromVersion 到最新版本的所有待执行迁移步骤
 */
export function getPendingMigrations(fromVersion) {
 ...
 */
export function applySchemaMigrations(task) {
...
export const SCHEMA_MIGRATIONS = [
    ...
    },
    {
        ...
        migrate(task) {
            ...
            let changed = false;
            // 迁移 pipeline 中间状态
            if (task.status in PIPELINE_STATUS_MIGRATION_MAP) {
            ...
            for (let i = 0; i < task.history.length; i++) {
                ...
                if (entry.action === 'verdict' && entry.newValue && typeof entry.newValue === 'string') {
                    if (!VALID_VERDICT_ACTIONS.includes(entry.newValue)) {
            ...
            if (task.verification) {
                ...
                if (verification.verdictAction && typeof verification.verdictAction === 'string') {
                    if (!VALID_VERDICT_ACTIONS.includes(verification.verdictAction)) {
    ...
    },
    {
        ...
        migrate(task) {
            const details = [];
            let changed = false;
            const { inferCheckpointPrefix, VALID_CHECKPOINT_PREFIXES } = require('../utils/validation-rules/checkpoint-rules.js');
            ...
            for (const cp of checkpoints) {
                ...
                const hasValidPrefix = VALID_CHECKPOINT_PREFIXES.some((prefix) => trimmed.startsWith(prefix.toLowerCase()));
                if (!hasValidPrefix) {
 ...
 * 验证历史记录条目格式
 */
export function validateHistoryEntry(entry, index) {
    const errors = [];
 ...
 * 验证需求变更历史条目格式
 */
export function validateRequirementHistoryEntry(entry, index) {
    const errors = [];
    if (!entry || typeof entry !== 'object') {
 ...
 * 验证任务 ID 格式
 */
export function validateTaskIdFormat(id) {
    const errors = [];
 ...
 * 验证状态值是否有效
 */
export function isValidStatusValue(status) {
    return VALID_STATUSES.includes(status);
}
/**
 * 验证类型值是否有效
 */
export function isValidTypeValue(type) {
    return VALID_TYPES.includes(type);
}
/**
 * 验证优先级值是否有效
 */
export function isValidPriorityValue(priority) {
 ...
 */
export async function performQualityCheck(cwd = process.cwd(), aiOptions) {
 ...
 * 显示内容质量检测结果
 */
export function showQualityReport(scores, options = {}) {
 ...
 */
async function detectSemanticDuplicates(tasks, cwd) {
    if (tasks.length < 2)
        return [];
 ...
 */
async function assessStalenessWithAI(task, cwd) {
    const result = await withAIEnhancement({
        enabled: true,
        aiCall: () => new AIMetadataAssistant(cwd).assessStaleness(task, { cwd }),
        fallback: { isStale: false, stalenessScore: 0, suggestedAction: 'keep', reason: '', aiUsed: false },
 ...
 */
function getHarnessReportDir(taskId, cwd) {
 ...
 */
export function checkReportStatusConsistency(taskId, task, cwd) {
 ...
 */
export function checkCheckpointConsistency(taskId, task) {
    if (normalizeStatus(task.status) !== 'resolved')
        return null;
 ...
 */
export function checkCheckpointValidation(taskId, task) {
    const issues = [];
 ...
 * 6. 质量建议生成
 */
export async function checkQualityGateIssues(taskId, task, cwd, config) {
    const issues = [];
    // 调用完整的质量门禁检查
    const qualityConfig = {
 ...
 */
export function checkMissingPipelineEvidence(taskId, task, cwd) {
    const currentStatus = normalizeStatus(task.status);
    const reportDir = getHarnessReportDir(taskId, cwd);
    // 定义每个中间状态所需的前置报告
    const STATUS_REQUIRED_REPORTS = {
        wait_review: [
 ...
 */
export function shouldTriggerAIInference(task, layer1Issues, cwd) {
    const currentStatus = normalizeStatus(task.status);
    // CP-2: 终态任务不需要 AI 推断
 ...
 * CP-9: 要求输出结构化 JSON
 */
export function buildStatusInferencePrompt(task, layer1Findings) {
    const currentStatus = normalizeStatus(task.status);
    // CP-7: 任务基本信息
    const taskInfo = [
        `## 任务基本信息`,
    ...
    const historySection = task.history && task.history.length > 0
        ? [
            ...
            ...task.history.slice(-10).map((h, i) => `${i + 1}. [${h.timestamp}] ${h.action}${h.field ? ` (${h.field}: ${h.oldValue} → ${h.newValue})` : ''}${h.reason ? ` 原因: ${h.reason}` : ''}`),
        ].join('\n')
        : '## 历史记录: 无';
    // 转换记录
 ...
 */
export async function runAIStatusInference(task, layer1Findings, cwd) {
 ...
 */
export async function detectStatusInferenceIssues(task, cwd, aiOptions) {
    const issues = [];
    // CP-15: 先运行 Layer 1 确定性规则
    ...
    if (!aiEnabled)
        return issues;
    if (!shouldTriggerAIInference(task, issues, cwd))
        return issues;
    // CP-17: 需要时调用 AI 推断
    ...
    if (aiResult.inferredStatus !== currentStatus && aiResult.confidence >= 0.6) {
        ...
        issues.push({
            ...
            details: {
                ...
                layer1Issues: issues.filter(i => i.type === 'report_status_mismatch' ||
                    i.type === 'checkpoint_status_mismatch' ||
                    i.type === 'missing_pipeline_evidence').length,
            },
        });
    }
 ...
 */
export async function analyzeProject(cwd = process.cwd(), includeArchived = false, aiOptions) {
    ...
    for (const task of filteredTasks) {
        ...
        if (task.verification) {
            ...
            if (verification.verdictAction && typeof verification.verdictAction === 'string') {
                if (!VALID_VERDICT_ACTIONS.includes(verification.verdictAction)) {
                    invalidVerdictActions.push(verification.verdictAction);
                }
            }
        }
        ...
        // 2.7 检测历史记录中对废弃状态的引用
        const deprecatedStatusRefs = [];
        for (const entry of task.history || []) {
            if (entry.oldValue === 'reopened' || entry.newValue === 'reopened' ||
                entry.oldValue === 'needs_human' || entry.newValue === 'needs_human') {
        ...
        if ((task.reopenCount ?? 0) > 0) {
            const reopenHistoryCount = task.history?.filter((h) => (h.action === 'status_change' && h.newValue === 'reopened') ||
                (h.field === 'status' && h.oldValue === 'resolved' && h.newValue === 'open')).length || 0;
            const reopenTransitionCount = task.transitionNotes?.filter((note) => note.fromStatus === 'resolved' && (note.toStatus === 'open' || note.toStatus === 'reopened')).length || 0;
        ...
        if (task.checkpoints && task.checkpoints.length > 0) {
            const manualCheckpoints = task.checkpoints.filter(cp => cp.verification?.method === 'manual');
        ...
        if (task.history && Array.isArray(task.history)) {
            const statusEntries = task.history.filter((entry) => entry.field === 'status' && entry.oldValue && entry.newValue);
            const existingNotes = task.transitionNotes || [];
            for (const entry of statusEntries) {
                const hasMatchingNote = existingNotes.some((note) => note.fromStatus === entry.oldValue && note.toStatus === entry.newValue);
                if (!hasMatchingNote) {
                    // 推断 author
        ...
        if (normalizedStatus === 'in_progress') {
            ...
            if (fs.existsSync(omcStateDir)) {
                ...
                for (const stateFile of pipelineStateFiles) {
                    try {
                        const stateData = JSON.parse(fs.readFileSync(path.join(omcStateDir, stateFile), 'utf-8'));
            ...
            if (!hasActivePipeline) {
                ...
                const completedCheckpoints = task.checkpoints?.filter(cp => cp.status === 'completed').length || 0;
 ...
 * CP-1: 支持 AI 增强分析选项
 */
export async function showAnalysis(options = {}, cwd = process.cwd()) {
    // CP-3: 模块日志 + 埋点初始化
 ...
 */
export async function showStatus(options = {}, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 */
export function calculateReopenStats(tasks) {
 ...
 * 从验收标准提取关键词
 */
export function extractKeywordsFromCriteria(criteria) {
    const keywords = [];
    // 移除常见的停用词
 ...
 * 搜索代码库以找到与验收标准相关的文件
 */
function searchCodebaseForCriteria(criteria, cwd) {
    const results = [];
 ...
 * 基于代码库搜索结果生成智能检查点
 */
function generateSmartCheckpoints(taskId, criteria, searchResults, index) {
 ...
 * 基于搜索结果智能推断验证方法
 */
function inferSmartVerificationMethod(criteria, searchResults) {
 ...
 * 推断验证命令
 */
function inferVerificationCommands(criteria, searchResults) {
    const commands = [];
    const lowerCriteria = criteria.toLowerCase();
    // 基于验收标准推断命令
    ...
    if (searchResults.length > 0) {
        // 如果匹配的是测试文件
        const hasTestFiles = searchResults.some(r => r.filePath.includes('test') || r.filePath.includes('spec'));
 ...
 * 获取 Contract 文件路径
 */
function getContractPath(taskId, cwd) {
 ...
 */
function generateDefaultCheckpoints(taskTitle, taskDescription) {
 ...
 */
function generateTitleBasedCheckpoints(taskTitle) {
    const checkpoints = [];
 ...
 */
export function extractAcceptanceCriteriaFromDescription(description) {
    const criteria = [];
    // 尝试提取列表项
 ...
 * 修复单个任务的检查点
 */
function fixTaskCheckpoints(taskId, cwd) {
    ...
    const checkpoints = generateCheckpointsFromCriteria(taskId, acceptanceCriteria, task.title, cwd, task.description);
 ...
 * 修复所有任务的检查点
 */
export async function fixCheckpoints(cwd = process.cwd(), options = {}) {
...
function classifyBug(fields) {
    ...
    let classification = { category: 'unknown', subcategory: 'unclassified', confidence: 0.3 };
 ...
 */
function generateBugAnalysisMarkdown(fields, logContext, analysis, reportTimestamp) {
    const severityLabel = {
        critical: '🔴 紧急',
        high: '🟠 高',
        medium: '🟡 中',
 ...
 */
function generateRequirementFromAnalysis(fields, analysis) {
    const severityLabel = {
        critical: '紧急',
        high: '高',
        medium: '中',
 ...
 * 导出训练数据为 JSONL 格式
 */
function exportTrainingDataToJsonl(fields, analysis, exportPath) {
    const trainingEntry = {
 ...
 * @param options - 分析选项
 */
export async function analyzeBugReport(bugReportPath, cwd = process.cwd(), options = {}) {
