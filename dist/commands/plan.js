...
import { readPlan, getOrCreatePlan, writePlan, addTaskToPlan, removeTaskFromPlan, clearPlan, areDependenciesCompleted, getExecutableTasks, detectMissingSubtasks, } from '../utils/plan';
...
import { runQualityGate } from '../utils/quality-gate-registry';
import { classifyFileToLayer, AIMetadataAssistant } from '../utils/ai-metadata';
...
import { topologicalSortDFS, findComponentsUnionFind, DependencyGraph, validatePlanOperation } from '../utils/dependency-graph';
// ============== 查询解析与过滤 ==============
// 停用词
 ...
 */
export function inferArchitectureLayer(task) {
 ...
 */
export function buildTaskChains(tasks, cwd, precomputedDeps) {
    if (tasks.length === 0)
        return [];
 ...
 * 同一优先级桶内的不同链标记为可并行
 */
export function buildBatches(sortedChains) {
    const priorityNames = {
        0: 'P0', 1: 'P1', 2: 'P2', 3: 'P3',
        4: 'Q1', 5: 'Q2', 6: 'Q3', 7: 'Q4',
    };
 ...
 * 生成 AI 友好的输出
 */
function generateAIOutput(chains, originalCount, filteredCount, batches, query, keywords, genOptions, subtaskWarnings) {
    const priorityNames = {
        0: 'P0', 1: 'P1', 2: 'P2', 3: 'P3',
        4: 'Q1', 5: 'Q2', 6: 'Q3', 7: 'Q4',
    };
 ...
 * 显示执行计划
 */
export function showPlan(json = false, cwd = process.cwd()) {
 ...
 * 添加任务到计划
 */
export function addTask(taskId, afterId, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 * 从计划移除任务
 */
export function removeTask(taskId, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 * 清空计划
 */
export async function clearPlanCmd(force = false, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 */
export async function recommendPlan(options = {}, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
    ...
    const keywordsForOutput = filter.type === 'keywords'
        ...
        : [filter.pattern];
    const aiOutput = generateAIOutput(sortedChains, activeTasks.length, filteredTasks.length, cachedBatches, options.query, keywordsForOutput, { smart: options.smart }, missingSubtaskWarnings);
    // JSON 格式输出（AI 友好）
