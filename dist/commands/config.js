...
import { DEFAULT_LOGGING, DEFAULT_AI, DEFAULT_TRAINING, DEFAULT_GIT_HOOK, } from '../types/config.js';
/**
 * 确保配置文件包含所有默认配置项
 * 缺失的配置项自动写入默认值
 */
export function ensureConfigDefaults(config) {
    const result = { ...config };
    // logging 配置完整性
    if (!result.logging) {
 ...
 * 提示词模板标注使用默认或自定义
 */
export function listConfig(cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
 ...
 * 获取指定配置值
 */
export function getConfig(key, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 * - 数值/布尔类型严格校验
 */
export function setConfig(key, value, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
