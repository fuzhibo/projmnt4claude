...
import { generateNewTaskId, readTaskMeta, writeTaskMeta, getAllTasks, taskExists, renameTask, } from '../utils/task';
import { parseCheckpointsWithIds, syncCheckpointsToMeta, updateCheckpointStatus, getCheckpointDetail, listCheckpoints, filterLowQualityCheckpoints, } from '../utils/checkpoint';
import { createDefaultTaskMeta, isValidTaskId, validateCheckpointVerification, normalizeStatus, normalizePriority, inferTaskType, } from '../types/task';
import * as crypto from 'crypto';
import { SEPARATOR_WIDTH } from '../utils/format';
import { DependencyGraph, validateNewTaskDeps } from '../utils/dependency-graph';
import { inferDependencies } from '../utils/dependency-engine';
import { generateStructuredDescription, extractStructuredInfo, inferCheckpointsFromDescription, inferRelatedFiles, } from '../utils/description-template';
import { extractFilePaths } from '../utils/quality-gate';
import { writeBatchUpdateLog, detectOperationSource, formatLogList, queryBatchUpdateLogs, showLogSummary, } from '../utils/batch-update-logger';
/** 历史记录最大显示条数 */
 ...
 */
export function hasValidCheckpoints(checkpointPathOrContent, isContent = false) {
    let content;
 ...
 */
export function displayCheckpointCreationWarning(taskId, cwd) {
    console.log('');
 ...
 */
export function displayCheckpointVerificationWarnings(warnings) {
 ...
 * 返回不存在的文件列表
 */
function findMissingFiles(filePaths, cwd) {
    const missing = [];
 ...
 */
async function validateFileReferences(description, title, nonInteractive, cwd) {
    const source = `${title || ''}\n${description || ''}`;
 ...
 * 支持交互模式和非交互模式
 */
export async function createTask(options = {}, cwd = process.cwd()) {
    ...
    if (options.nonInteractive && options.title) {
        ...
        const taskType = options.type
            ? options.type
            : (options.aiEnhancement ? inferTaskType(options.title) : 'feature');
        ...
        const fileValidation = await validateFileReferences(options.description, options.title, true, cwd);
        ...
        if ((options.aiEnhancement || options.potentialDependencies) && options.description) {
            ...
            const inferredDeps = inferDependencies(options.description, existingTasks, { keywordHints: options.potentialDependencies });
 ...
 * 支持多种输出格式和过滤选项
 */
export function listTasks(options = {}, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 */
export function showTask(taskId, options = {}, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 * 精简输出格式
 */
function showTaskCompact(task, cwd) {
 ...
 * 按显示宽度截断字符串
 */
function truncateByDisplayWidth(str, maxDisplayWidth) {
    let result = '';
    let currentWidth = 0;
    for (const char of str) {
 ...
 * 按显示宽度填充空格（用于对齐）
 */
function padByDisplayWidth(str, targetWidth) {
 ...
 * 新面板格式 - 紧凑美观
 */
function showTaskPanel(task, options, cwd) {
...
function groupConsecutiveStatusChanges(history) {
    const groups = [];
    const GROUP_TIME_GAP_MS = 5 * 60 * 1000; // 5分钟内的连续状态变更合并
    for (const entry of history) {
        const lastGroup = groups[groups.length - 1];
        if (lastGroup?.type === 'status-flow' &&
            entry.field === 'status' &&
 ...
 * 经典格式 - 详细输出
 */
function showTaskClassic(task, options, cwd) {
 ...
 * 支持子任务状态同步
 */
export async function updateTask(taskId, options, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
            ...
            if (options.syncChildren) {
                ...
                for (const childId of childTasks) {
                    ...
                    if (childTask && childTask.status !== 'resolved' && childTask.status !== 'closed') {
                        childTask.status = options.status;
                        // 添加历史记录
 ...
 */
export async function submitTask(taskId, options = {}, cwd = process.cwd()) {
    // 输出废弃警告
    console.warn('');
 ...
 * 执行验证并更新任务状态
 */
export async function validateTask(taskId, options = {}, cwd = process.cwd()) {
    // 动态导入避免循环依赖
 ...
 */
export async function deleteTask(taskId, force = false, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 */
export function purgeTasks(options = {}, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 */
export function addDependency(taskId, depId, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 */
export function removeDependency(taskId, depId, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 */
function wouldCreateCycle(taskId, depId, cwd) {
    const allTasks = getAllTasks(cwd);
    const graph = DependencyGraph.fromTasks(allTasks);
 ...
 * 帮助用户理解任务状态流转
 */
export function showStatusGuide() {
    console.log('');
    console.log('━'.repeat(SEPARATOR_WIDTH));
 ...
 */
export async function completeTask(taskId, options = {}, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 * 查看任务的完整变更历史
 */
export function showTaskHistory(taskId, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 * 添加历史记录条目
 */
export function addHistoryEntry(taskId, entry, cwd = process.cwd()) {
    const task = readTaskMeta(taskId, cwd);
    if (!task)
        return;
    if (!task.history) {
 ...
 */
export async function executeTask(taskId, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 * 支持非交互模式 (--yes)
 */
export async function completeCheckpoint(taskId, options = {}, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 * 验证检查点并生成令牌 (P1-003)
 */
export async function verifyCheckpoint(taskId, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 * 添加子任务
 */
export async function addSubtask(parentId, title, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 */
export async function syncChildren(parentTaskId, options = {}, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
 ...
 */
export async function updateCheckpoint(taskId, checkpointId, action, options = {}, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
 ...
 * 列出任务的所有检查点
 */
export async function listTaskCheckpoints(taskId, options = {}, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化');
        process.exit(1);
    }
 ...
 */
export async function splitTask(parentId, options = {}, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 */
export function searchTasks(keyword, options = {}, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 * 支持按状态、优先级、类型分组统计
 */
export function countTasks(options = {}, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 */
export function showStatus(options = {}, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 */
export async function batchUpdateTasks(options = {}, cwd = process.cwd()) {
    // 记录操作开始时间
 ...
 */
export function showBatchUpdateLogs(options = {}, cwd = process.cwd()) {
    if (options.summary) {
 ...
 */
export function generateCheckpointTemplate(taskId, options = {}, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
 ...
 */
export function renameTaskCommand(oldTaskId, newTaskId, cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        console.error('错误: 项目未初始化。请先运行 `projmnt4claude setup`');
        process.exit(1);
    }
