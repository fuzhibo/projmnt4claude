...
import { readTaskMeta, getAllTaskIds, buildTaskVerification, renameTask, validateStatusTransition, StatusTransitionError, } from '../utils/task';
import { validatedWriteTaskMeta } from '../utils/task-validation';
import { parseTaskId, generateTaskId, normalizeStatus, normalizePriority, PIPELINE_STATUS_MIGRATION_MAP, } from '../types/task';
import { DependencyGraph } from '../utils/dependency-graph';
import { VALID_VERDICT_ACTIONS } from '../types/harness';
import { SEPARATOR_WIDTH } from '../utils/format';
import { inferCheckpointPrefix, VALID_CHECKPOINT_PREFIXES, } from '../utils/validation-rules/checkpoint-rules.js';
import { syncCheckpointsToMeta } from '../utils/checkpoint.js';
import { applySchemaMigrations, analyzeProject, fixCheckpoints, performQualityCheck, showQualityReport, } from './analyze';
 ...
 * 判断 slug 是否无意义
 */
export function isMeaninglessSlug(slug) {
    if (!slug || slug.length === 0)
        return true;
    // 哈希生成的标识: t + 短字母数字 (如 t1a2b3c)
    if (/^t[0-9a-z]+$/.test(slug) && slug.length <= 12)
        return true;
    // 通用词: task 或 task- 前缀
    if (slug === 'task' || slug.startsWith('task-'))
        return true;
    return false;
}
/**
 ...
 */
function extractSlugFromTask(task) {
 ...
 * @returns 验证结果
 */
function setTaskStatusValidated(task, newStatus, context) {
 ...
 */
export async function fixSingleIssue(issue, cwd, nonInteractive) {
    ...
    switch (issue.type) {
        ...
        case 'null_array_field': {
            const fields = issue.details?.fields ?? [];
            if (fields.length === 0)
                return 'skipped';
        ...
        case 'pipeline_status_migration': {
            ...
            const oldStatus = task.status;
            const targetStatus = issue.details?.targetStatus;
            ...
            if (task.verification) {
                ...
                if (verification.verdictAction && typeof verification.verdictAction === 'string') {
                    if (!VALID_VERDICT_ACTIONS.includes(verification.verdictAction)) {
                        const oldVal = verification.verdictAction;
        ...
        case 'invalid_subtask_ref': {
            ...
            if (task.subtaskIds && issue.details?.subtaskId) {
                const invalidId = issue.details.subtaskId;
        ...
        case 'invalid_dependency_ref': {
            ...
            if (!removedAny && task.dependencies && issue.details?.dependencyId) {
                const invalidId = issue.details.dependencyId;
                const oldLength = task.dependencies.length;
        ...
        case 'parent_child_mismatch': {
            ...
            if (issue.details?.subtaskId && issue.details?.expectedParentId) {
                const subtaskId = issue.details.subtaskId;
                const subtask = readTaskMeta(subtaskId, cwd);
                if (subtask) {
                    subtask.parentId = issue.details.expectedParentId;
        ...
        case 'meaningless_id': {
            ...
            const taskType = (task.type || idInfo.type || 'feature');
            const taskPriority = (task.priority || idInfo.priority);
        ...
        case 'manual_verification': {
            ...
            if (task.checkpoints && issue.details?.checkpointIds) {
                ...
                for (const cpId of issue.details.checkpointIds) {
                    const cp = task.checkpoints.find(c => c.id === cpId);
                    if (cp && cp.verification && cp.verification.method === 'manual') {
        ...
        case 'abandoned_residual': {
            ...
            const tasksToDelete = issue.details?.tasks || [];
        ...
        case 'file_not_found': {
            ...
            if (issue.details?.missingFiles) {
                console.log(`   不存在的文件: ${issue.details.missingFiles.join(', ')}`);
            }
            console.log(`   建议: ${issue.suggestion}`);
            return 'unfixable';
        }
        case 'missing_transition_note': {
            ...
            const historyEntry = issue.details?.historyEntry;
            if (!historyEntry) {
            ...
            if (analysis)
                noteParts.push(`分析: ${analysis}`);
            if (evidence)
                noteParts.push(`证据: ${evidence}`);
            ...
            task.transitionNotes.push({
                ...
                fromStatus: historyEntry.oldValue,
                toStatus: historyEntry.newValue,
                note,
                author,
            });
            validatedWriteTaskMeta(task, cwd);
        ...
        case 'interrupted_task': {
            const suggestedStatus = issue.details?.suggestedStatus;
            if (!suggestedStatus) {
            ...
            if (nonInteractive || !process.stdin.isTTY) {
                const oldStatus = task.status;
                setTaskStatusValidated(task, suggestedStatus, 'interrupted-task-auto');
                task.updatedAt = new Date().toISOString();
                // writeTaskMeta 自动监听 status 字段变更并生成 history 记录
                // 添加 transitionNote
                if (!task.transitionNotes)
                ...
                task.transitionNotes.push({
                    ...
                    note: `中断任务自动修复: ${issue.details?.interruptedDays || 0} 天无活跃 Pipeline`,
                    author: 'analyze',
                });
                validatedWriteTaskMeta(task, cwd);
            ...
            if (response.apply) {
                const oldStatus = task.status;
                setTaskStatusValidated(task, suggestedStatus, 'interrupted-task-manual');
                task.updatedAt = new Date().toISOString();
                // writeTaskMeta 自动监听 status 字段变更并生成 history 记录
                if (!task.transitionNotes)
                    task.transitionNotes = [];
                task.transitionNotes.push({
                    ...
                    fromStatus: oldStatus,
                    toStatus: suggestedStatus,
                    note: `中断任务手动确认修复: ${issue.details?.interruptedDays || 0} 天无活跃 Pipeline`,
                    author: 'analyze',
                });
                validatedWriteTaskMeta(task, cwd);
                console.log(`  ✅ 已将任务 ${issue.taskId} 状态从 ${oldStatus} 修改为 ${suggestedStatus}`);
        ...
        case 'low_checkpoint_coverage': {
            ...
            if (issue.details?.coverageRate != null) {
                console.log(`   当前覆盖率: ${(issue.details.coverageRate * 100).toFixed(1)}%`);
            }
        ...
        case 'low_quality': {
            ...
            const deductions = issue.details?.deductions ?? [];
        ...
        case 'deprecated_status_reference': {
            ...
            for (const entry of task.history || []) {
                if (entry.oldValue === 'reopened' || entry.oldValue === 'needs_human') {
                    entry.oldValue = 'open';
                    fixedAny = true;
                }
                if (entry.newValue === 'reopened') {
                    entry.newValue = 'open';
                    fixedAny = true;
                }
                if (entry.newValue === 'needs_human') {
                    entry.newValue = 'open';
                    fixedAny = true;
                }
            }
            for (const note of task.transitionNotes || []) {
                if (note.fromStatus === 'reopened' || note.fromStatus === 'needs_human') {
                    note.fromStatus = 'open';
                    fixedAny = true;
                }
                if (note.toStatus === 'reopened' || note.toStatus === 'needs_human') {
                    note.toStatus = 'open';
                    fixedAny = true;
                }
            }
        ...
        case 'report_status_mismatch': {
            ...
            const impliedStatus = issue.details?.impliedStatus;
            if (!impliedStatus) {
 ...
 */
export async function fixIssues(cwd = process.cwd(), options = {}, existingResult) {
    ...
    if (skippedCount > 0)
        console.log(`⏭️  跳过 ${skippedCount} 个问题`);
    if (unfixableCount > 0)
        console.log(`⚠️  ${unfixableCount} 个问题无法自动修复`);
    console.log('━'.repeat(SEPARATOR_WIDTH));
    return { fixed: fixedCount, skipped: skippedCount, unfixable: unfixableCount };
}
function emptyStageResult(summary) {
 ...
 *   --fix --quality-only → 仅5
 */
export async function fixPipeline(cwd = process.cwd(), options = {}) {
    const pipelineStart = Date.now();
    const { nonInteractive = false, rulesOnly = false, checkpointsOnly = false, qualityOnly = false, noAi = false, aiOptions = {}, compact = false, json = false, threshold = 60, taskId, } = options;
    ...
    if (runStage3) {
        ...
        if (aiOptions.deepAnalyze && !aiOptions.noAi) {
            try {
                ...
                const aiIssues = analysisResult.issues.filter(i => i.type === 'semantic_duplicate' || i.type === 'missing_inferred_dependency');
                const duration = Date.now() - start;
                stage3Result = {
                    executed: true,
                    skipped: false,
 ...
 * @returns 修复结果列表
 */
export async function applyStatusInferenceFix(cwd = process.cwd(), nonInteractive = true) {
    // 动态导入避免循环依赖（analyze.ts 已导入本模块）
    const { checkReportStatusConsistency, checkCheckpointConsistency, checkMissingPipelineEvidence, } = await import('./analyze');
    ...
    for (const task of tasks) {
        ...
        if (reportIssue) {
            ...
            results.push({
                ...
                applied: fixResult === 'fixed',
                oldValue: task.status,
                newValue: reportIssue.details?.impliedStatus,
