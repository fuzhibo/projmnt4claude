...
import { generateNewTaskId, writeTaskMeta, readTaskMeta, getAllTasks, addSubtaskToParent, } from '../utils/task';
import { extractAffectedFiles, extractFilePaths, checkQualityGate, validateBasicFields, validateFilesExist, DEFAULT_QUALITY_GATE_CONFIG } from '../utils/quality-gate';
import { hasValidCheckpoints, displayCheckpointCreationWarning, createTask } from './task';
...
import { inferDependencies as inferDependenciesUnified } from '../utils/dependency-engine';
...
import { AIMetadataAssistant, classifyFileToLayer, groupFilesByLayer, sortFilesByLayer, LAYER_DEFINITIONS } from '../utils/ai-metadata';
 ...
 * 从自然语言需求创建任务
 */
export async function initRequirement(description, cwd = process.cwd(), options = {}) {
    ...
    if (shouldDecomposeOption && shouldDecompose(description)) {
        ...
        if (decomposition.decomposable && decomposition.items.length >= 2) {
            ...
            if (confirmDecompose) {
                ...
                const taskIdMap = new Map(); // 索引 -> 任务ID 映射
                console.log('');
                console.log('━'.repeat(SEPARATOR_WIDTH));
                console.log('📝 正在创建任务...');
                console.log('━'.repeat(SEPARATOR_WIDTH));
 ...
 * - 描述长度: 过长的描述往往暗示范围不清晰
 */
export function assessComplexity(description, analysis) {
    const signals = [];
    // 1. 文件数量信号
    const files = extractFilePaths(description);
    ...
    const estimatedMinutes = Math.max(5 + fileCount * 3 + workItemCount * 2, Math.ceil(descLength / 100) // 备选: 每百字 1 分钟
    );
    // 超过 15 分钟强制标记为 high
...
function generateSplitSuggestions(description, files, workItemCount, totalMinutes, analysis) {
    ...
    const indexMap = new Map(); // old suggestions index → new finalSuggestions index
