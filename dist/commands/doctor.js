...
import { isInitialized, getProjectDir, getTasksDir, getToolboxDir, getLogsDir, } from '../utils/path';
 ...
 * 运行环境诊断
 */
export async function runDoctor(fix = false, cwd = process.cwd()) {
    console.log('');
 ...
 * 检查目录结构完整性
 */
function checkDirectoryStructure(cwd) {
    const results = [];
    const projectDir = getProjectDir(cwd);
    const requiredDirs = [
...
function checkPluginInstallationScope(cwd) {
    ...
    try {
        ...
        const projectScopedInstalls = installations.filter((inst) => inst.scope === 'project');
 ...
 */
function checkLoggingModule(cwd) {
    const results = [];
 ...
 */
function checkDeprecatedStatuses(cwd) {
    const results = [];
    const tasksDir = getTasksDir(cwd);
    if (!fs.existsSync(tasksDir)) {
        return [{
 ...
 */
function checkDeprecatedHooks(cwd) {
    const results = [];
 ...
 * 修复问题
 */
async function fixIssues(issues, cwd) {
    const projectDir = getProjectDir(cwd);
            ...
            if (pluginRoot) {
                ...
                if (fs.existsSync(commandsSourceDir)) {
                    ...
                    for (const file of commandFiles) {
                        fs.copyFileSync(path.join(commandsSourceDir, file), path.join(commandsTargetDir, file));
                    }
 ...
 */
export async function runBugReport(cwd = process.cwd()) {
    console.log('');
    console.log('━'.repeat(SEPARATOR_WIDTH));
 ...
 * 提供更深入的问题检测和修复建议。
 */
export async function runDoctorDeep(cwd = process.cwd()) {
    console.log('');
    console.log('━'.repeat(SEPARATOR_WIDTH));
