...
// 隔离测试环境工具
import { createIsolatedTestEnv, createTaskDir, readTaskMeta, writeTaskMeta, taskExists, getAllTaskIds, createArchivedTask, createTestTasks, createTaskDependency, createTestLifecycle, resetTestEnv, } from '../utils/test-env.js';
 ...
 * 创建测试任务元数据
 */
function createTestTaskMeta(id, overrides) {
 ...
 * 将任务写入磁盘
 */
function writeTaskToDisk(tasksDir, task) {
 ...
 * 从磁盘读取任务
 */
function readTaskFromDisk(tasksDir, taskId) {
...
describe('场景 2: 任务创建与管理', () => {
    ...
    test('S2.2: 使用 writeTaskMeta 写入任务元数据', () => {
        const task = createTestTaskMeta('TASK-002', {
            title: '自定义任务',
            priority: 'P1',
        });
