...
function setupTaskDir(tmpDir, taskId, status = 'open') {
...
describe('handleVerdictBasedTransition status transitions', () => {
     ...
     */
    async function runVerdictTransition(phase, action, initialStatus = 'wait_qa') {
        ...
        const result = await assemblyLine.handleVerdictBasedTransition(taskId, record, state, addTimeline, phase, action);
        return result;
    }
    /**
    ...
    test('CP-13: retest does not set resumeFrom (deprecated mechanism)', async () => {
        ...
        const state = createDefaultRuntimeState(createTestConfig(tmpDir));
        const addTimeline = () => { };
        await assemblyLine.handleVerdictBasedTransition(taskId, record, state, addTimeline, 'qa', 'retest');
    ...
    test('CP-13: reevaluate does not set resumeFrom (deprecated mechanism)', async () => {
        ...
        const state = createDefaultRuntimeState(createTestConfig(tmpDir));
        const addTimeline = () => { };
        await assemblyLine.handleVerdictBasedTransition(taskId, record, state, addTimeline, 'evaluation', 'reevaluate');
        expect(state.resumeFrom.has(taskId)).toBe(false);
    });
    ...
    test('CP-11: retest falls back to redevelop when QA retries exhausted', async () => {
        ...
        const addTimeline = () => { };
        const result = await assemblyLine.handleVerdictBasedTransition(taskId, record, state, addTimeline, 'qa', 'retest');
    ...
    test('CP-12: reevaluate falls back to redevelop when max attempts reached', async () => {
        ...
        const addTimeline = () => { };
        const result = await assemblyLine.handleVerdictBasedTransition(taskId, record, state, addTimeline, 'evaluation', 'reevaluate');
        // Should fall back to redevelop → in_progress
        expect(result.finalStatus).toBe('in_progress');
    });
    ...
    test('CP-17: resolve transitions to resolved', async () => {
        ...
        const result = await assemblyLine.handleVerdictBasedTransition(taskId, record, state, addTimeline, 'evaluation', 'resolve');
    ...
    test('CP-17: escalate_human transitions to open', async () => {
        ...
        const result = await assemblyLine.handleVerdictBasedTransition(taskId, record, state, addTimeline, 'evaluation', 'escalate_human');
    ...
    test('boundary: redevelop marks failed when dev retries exhausted', async () => {
        ...
        const addTimeline = () => { };
        const result = await assemblyLine.handleVerdictBasedTransition(taskId, record, state, addTimeline, 'qa', 'redevelop');
    ...
    test('boundary: minor_fix falls back to redevelop when phase retries exhausted', async () => {
        ...
        const addTimeline = () => { };
        const result = await assemblyLine.handleVerdictBasedTransition(taskId, record, state, addTimeline, 'qa', 'minor_fix');
        // Falls back to redevelop → in_progress
        expect(result.finalStatus).toBe('in_progress');
    });
    ...
    test('boundary: redevelop requeues task and increments counters', async () => {
        ...
        const state = createDefaultRuntimeState(createTestConfig(tmpDir));
        const addTimeline = () => { };
        await assemblyLine.handleVerdictBasedTransition(taskId, record, state, addTimeline, 'qa', 'redevelop');
...
describe('QA pass → wait_evaluation transition', () => {
    test('CP-7: implementation calls ensureTransition to wait_evaluation after QA pass', () => {
        const sourceCode = fs.readFileSync(path.resolve(__dirname, '../utils/hd-assembly-line.ts'), 'utf-8');
    ...
    test('CP-8: qa checkpoint → nextPhaseAfter returns evaluation', () => {
        ...
        const nextPhase = al.nextPhaseAfter('qa');
...
describe('CP-5: prerequisite data rebuild from prevRecord', () => {
    ...
    test('prevRecord lookup finds latest record for task', () => {
        ...
        record1.codeReviewVerdict = {
            ...
            failedCheckpoints: [], reviewedAt: new Date().toISOString(), reviewedBy: 'code_reviewer',
        };
        record1.qaVerdict = {
            taskId, result: 'PASS', reason: 'OK', testFailures: [],
            failedCheckpoints: [], requiresHuman: false, humanVerificationCheckpoints: [],
            verifiedAt: new Date().toISOString(), verifiedBy: 'qa_tester',
        };
