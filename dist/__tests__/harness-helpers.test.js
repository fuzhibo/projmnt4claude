...
import { classifyExitResult, isRetryableError, parseStructuredResult, parseVerdictResult, filterCheckpoints, getReportDir, getReportPath, sleep, archiveReportIfExists, saveReport, runHeadlessClaude, runHeadlessClaudeWithRetry, DEFAULT_TIMEOUT_SECONDS, REVIEW_TIMEOUT_RATIO, parseDevReport, parseCodeReviewReport, parseQAReport, rebuildPrerequisiteData, } from '../utils/harness-helpers.js';
...
describe('saveReport', () => {
    ...
    test('throws on write failure', async () => {
        ...
        try {
            ...
            expect(error.message).toContain('保存报告失败');
        }
        finally {
...
describe('runHeadlessClaude', () => {
    ...
    test('writes prompt to stdin', async () => {
        ...
        const writes = [];
        child.stdin.write = (data) => { writes.push(data); };
        child.stdin.end = () => { };
        await runHeadlessClaude({
            prompt: 'hello world',
            allowedTools: ['Read'],
...
describe('runHeadlessClaudeWithRetry', () => {
    function setupMockSpawnSequence(results) {
        let callIndex = 0;
        spawnMock.mockImplementation(() => {
    ...
    test('returns success on first attempt', async () => {
        ...
        const result = await runHeadlessClaudeWithRetry({ prompt: 'test', allowedTools: ['Read'], timeout: 30, cwd: '/tmp' }, { maxAttempts: 3, baseDelay: 0.01 });
        expect(result.success).toBe(true);
    ...
    test('retries on network error and succeeds', async () => {
        ...
        const result = await runHeadlessClaudeWithRetry({ prompt: 'test', allowedTools: ['Read'], timeout: 30, cwd: '/tmp' }, { maxAttempts: 2, baseDelay: 0.01 });
        expect(result.success).toBe(true);
    });
...
describe('rebuildPrerequisiteData', () => {
    ...
    function writeCodeReviewReport() {
        const report = [
            '# 代码审核报告 - TASK-test-001',
            '',
            '**结果**: ✅ PASS',
