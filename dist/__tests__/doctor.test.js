...
function logContains(substr) {
    return consoleLogs.some(l => l.includes(substr));
}
// ── FS state (in-memory) ──────────────────────────────────
let fsState = {}; // null = directory
function fsReset() {
    fsState = {};
}
function fsSet(filePath, content) {
...
function fsRead(filePath) {
    if (!(filePath in fsState))
        throw new Error(`ENOENT: ${filePath}`);
    const v = fsState[filePath];
    if (v === null)
        throw new Error(`EISDIR: ${filePath}`);
    return v;
}
function fsIsDir(filePath) {
...
// ── Mock functions ─────────────────────────────────────────
const mockIsInitialized = mock((cwd) => fsExists(CONFIG_PATH));
const mockGetProjectDir = mock((cwd) => PROJECT_DIR);
const mockGetTasksDir = mock((cwd) => TASKS_DIR);
const mockGetToolboxDir = mock((cwd) => TOOLBOX_DIR);
const mockGetLogsDir = mock((cwd) => LOGS_DIR);
const mockGetAllTaskIds = mock((cwd) => {
    if (!fsExists(TASKS_DIR))
        return [];
    return fsReaddir(TASKS_DIR);
});
const mockReadConfig = mock((cwd) => {
...
const mockWriteConfig = mock((config, cwd) => {
    fsSet(CONFIG_PATH, JSON.stringify(config, null, 2));
});
const mockEnsureConfigDefaults = mock((config) => {
    return {
        ...config,
...
mock.module('../utils/path', () => ({
    ...
    getConfigPath: (cwd) => CONFIG_PATH,
    getTasksDir: mockGetTasksDir,
    getToolboxDir: mockGetToolboxDir,
    getLogsDir: mockGetLogsDir,
    getArchiveDir: (cwd) => path.join(PROJECT_DIR, 'archive'),
    ensureDir: (dir) => { fsMkdir(dir); },
}));
...
mock.module('../utils/log-analyzer', () => ({
    ...
    LogAnalyzerRegistry: class MockLogAnalyzerRegistry {
        analyzers = [];
        register(a) { this.analyzers.push(a); }
...
mock.module('fs', () => ({
    ...
    readFileSync: (p) => fsRead(p),
    writeFileSync: (p, content) => { fsSet(p, content); },
    readdirSync: (p) => fsReaddir(p),
    statSync: (p) => ({ size: fsStatSize(p), isDirectory: () => fsIsDir(p) }),
    mkdirSync: (p) => { fsMkdir(p); },
    copyFileSync: (src, dest) => { fsSet(dest, fsRead(src)); },
}));
// ── Import SUT ─────────────────────────────────────────────
