 ...
 * 创建临时测试目录结构
 */
export function createTestProjectStructure(mockFs, basePath = '/test-project') {
    // 创建项目目录结构
    ...
    // 创建基本配置文件
    mockFs.writeFileSync(`${basePath}/package.json`, JSON.stringify({ name: 'test-project', version: '1.0.0' }, null, 2));
}
/**
 * 模拟 fs 模块的辅助函数
 */
export function mockFsModule(mockFs) {
    return {
        ...fs,
        mkdirSync: (path, options) => mockFs.mkdirSync(path, options),
        writeFileSync: (path, content) => mockFs.writeFileSync(path, content),
        readFileSync: (path, encoding) => mockFs.readFileSync(path, encoding),
        existsSync: (path) => mockFs.existsSync(path),
        readdirSync: (path) => mockFs.readdirSync(path),
        rmSync: (path, options) => mockFs.rmSync(path, options),
