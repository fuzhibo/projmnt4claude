...
/** 在目录中创建 package.json */
function createPackageJson(dir, overrides = {}) {
...
describe('json()', () => {
    ...
    describe('正常流程', () => {
        ...
        it('update 增量更新数据', () => {
            ...
            const result = ops.update((data) => ({ ...data, count: data.count + 1 }));
...
describe('Pre', () => {
    ...
    describe('installPrePublish', () => {
        it('在 package.json 中添加 prepublishOnly 脚本', () => {
            ...
            const pkg = json(path.join(tempDir, 'package.json'));
            const data = pkg.read();
...
describe('集成测试: 完整发布流程', () => {
    ...
    it('json + toml 配置文件协同工作', () => {
        const pkgOps = json(path.join(tempDir, 'package.json'));
        ...
        pkgOps.update((data) => ({
            ...data,
            scripts: { ...data.scripts, build: 'bun build' },
        }));
