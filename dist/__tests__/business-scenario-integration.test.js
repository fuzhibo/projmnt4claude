...
import { createDefaultRuntimeState, createDefaultExecutionRecord, createDefaultDevReport, createDefaultSprintContract, DEFAULT_PHASE_RETRY_LIMITS, DEFAULT_HARNESS_CONFIG, } from '../types/harness.js';
import { createDefaultTaskMeta, normalizeStatus, normalizePriority, isValidTaskId, parseTaskId, inferTaskType, inferTaskPriority, generateTaskId, Pipeline, PHASE_ROLE_MAP, PIPELINE_INTERMEDIATE_STATUSES, PIPELINE_STATUS_MIGRATION_MAP, validateCheckpointVerification, CURRENT_TASK_SCHEMA_VERSION, } from '../types/task.js';
// Commands & utilities
...
import { FeedbackConstraintEngineImpl, JsonFeedbackTemplate, MarkdownFeedbackTemplate, jsonParseableRule, nonEmptyOutputRule, createJsonFeedbackEngine, createMarkdownFeedbackEngine, createSessionAwareEngine, } from '../utils/feedback-constraint-engine.js';
...
function setupProjectDir() {
    ...
    fs.writeFileSync(path.join(projDir, 'config.json'), JSON.stringify({ version: 1, tasks: { nextId: 1 } }), 'utf-8');
    return projDir;
}
function createTestTask(id, overrides) {
...
function createTestCheckpoint(id, overrides) {
    return {
        id,
...
function createPassedDevReport(taskId) {
    return {
        taskId,
        status: 'success',
        changes: ['src/foo.ts'],
...
function createPassedCodeReviewVerdict(taskId) {
    return {
        taskId,
        result: 'PASS',
...
function createFailedCodeReviewVerdict(taskId, reason = 'Code quality issues') {
    return {
        taskId,
        result: 'NOPASS',
        reason,
...
function createPassedQAVerdict(taskId) {
    return {
        taskId,
        result: 'PASS',
...
function createFailedQAVerdict(taskId, reason = 'Test failures found') {
    return {
        taskId,
        result: 'NOPASS',
        reason,
...
function createPassedVerdict(taskId, action) {
    return {
        taskId,
        result: 'PASS',
...
function createFailedVerdict(taskId, action = 'redevelop', reason = 'Not acceptable') {
    return {
        taskId,
        result: 'NOPASS',
        reason,
...
describe('Scenario 1: Task Full Lifecycle', () => {
    ...
    test('S1.3: transition through status lifecycle: open→in_progress→wait_review→wait_qa→resolved', () => {
        ...
        writeTaskToDisk(task);
        const statuses = ['in_progress', 'wait_review', 'wait_qa', 'resolved'];
...
describe('Scenario 7: Batch Execution & Git Commit', () => {
    test('S7.1: buildBatchAwareQueue with multiple batches', () => {
        const result = buildBatchAwareQueue(['T1', 'T2', 'T3', 'T4', 'T5'], [['T1', 'T2'], ['T3', 'T4'], ['T5']]);
...
describe('Cross-Module Integration', () => {
    test('X1: task lifecycle with phase history and resume action', () => {
        ...
        const resumePoint = Pipeline.determineResumePoint(loaded.phaseHistory, loaded.resumeAction);
