...
const mockReadTaskMeta = mock((id) => ({
    id,
    title: 'Test Task',
...
const mockFilterLowQuality = mock((cps) => ({ kept: cps, removed: [], reasons: new Map() }));
const mockWithAI = mock((opts) => Promise.resolve({ aiUsed: false }));
...
const mockExtractFilePaths = mock((desc) => {
...
function resetMocks() {
    ...
    mockReadTaskMeta.mockImplementation((id) => ({
