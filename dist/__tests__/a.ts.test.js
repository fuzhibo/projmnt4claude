...
import { resolveTemplate, loadPromptTemplate, PROMPT_TEMPLATE_NAMES, DEFAULT_TEMPLATES, DEFAULT_DEV_TEMPLATE, DEFAULT_CODE_REVIEW_TEMPLATE, DEFAULT_QA_TEMPLATE, DEFAULT_EVALUATION_TEMPLATE, DEFAULT_REQUIREMENT_TEMPLATE, DEFAULT_CHECKPOINTS_TEMPLATE, DEFAULT_QUALITY_TEMPLATE, DEFAULT_DUPLICATES_TEMPLATE, DEFAULT_STALENESS_TEMPLATE, DEFAULT_BUG_REPORT_TEMPLATE, DEFAULT_SEMANTIC_DEPENDENCY_TEMPLATE, } from '../utils/prompt-templates.js';
...
describe('resolveTemplate', () => {
    ...
    describe('边界条件处理', () => {
        ...
        it('应该正确处理值为 null 的情况', () => {
            const template = 'Value: {val}';
            const result = resolveTemplate(template, { val: null });
            expect(result).toBe('Value: {val}');
        });
    });
    describe('异常输入处理', () => {
