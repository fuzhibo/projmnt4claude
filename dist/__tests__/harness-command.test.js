...
// ---- Test helpers ----
function createProjectDir(tmpDir) {
    const projDir = path.join(tmpDir, '.projmnt4claude');
    fs.mkdirSync(projDir, { recursive: true });
    fs.writeFileSync(path.join(projDir, 'config.json'), JSON.stringify({ version: 1 }), 'utf-8');
    return projDir;
}
function createTaskMeta(projDir, taskId, overrides = {}) {
    ...
    fs.writeFileSync(path.join(taskDir, 'meta.json'), JSON.stringify(meta), 'utf-8');
}
function createPlanFile(tmpDir, tasks, batches) {
...
describe('harnessCommand: validation', () => {
    ...
    beforeEach(() => {
        ...
        runSpy = spyOn(AssemblyLine.prototype, 'run').mockResolvedValue(createMockSummary());
    });
    afterEach(() => {
    ...
    test('exits when project not initialized', async () => {
        ...
        expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('未初始化'));
    ...
    test('rejects quality score below 0', async () => {
        ...
        expect(exitSpy).toHaveBeenCalledWith(1);
        expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('require-quality'));
    });
    ...
    test('rejects negative maxRetries', async () => {
        ...
        expect(exitSpy).toHaveBeenCalledWith(1);
        expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('max-retries'));
    });
    ...
    test('rejects when no tasks available', async () => {
        ...
        expect(exitSpy).toHaveBeenCalledWith(1);
        expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('无法获取任务列表'));
    });
    test('rejects when plan file does not exist', async () => {
        try {
            await harnessCommand({ plan: 'nonexistent.json', dryRun: true }, tmpDir);
        }
        catch { }
        expect(exitSpy).toHaveBeenCalledWith(1);
        expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('计划文件不存在'));
    });
...
describe('harnessCommand: dry run', () => {
    ...
    test('shows task list without executing', async () => {
        ...
        await harnessCommand({ plan: planFile, dryRun: true, skipHarnessGate: true }, tmpDir);
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('试运行'));
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('TASK-1'));
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('TASK-2'));
        expect(exitSpy).not.toHaveBeenCalled();
    });
    test('shows batch info when batches present', async () => {
        ...
        const planFile = createPlanFile(tmpDir, ['TASK-A', 'TASK-B', 'TASK-C'], [['TASK-A', 'TASK-B'], ['TASK-C']]);
        await harnessCommand({ plan: planFile, dryRun: true, skipHarnessGate: true }, tmpDir);
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('批次 1'));
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('批次 2'));
        // Batch 1 has 2 tasks → parallelizable
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('可并行'));
    });
    ...
    test('displays config header with task count', async () => {
        ...
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('Harness Design'));
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('任务数量: 2'));
    });
    ...
    test('single-task batch is not marked parallelizable', async () => {
        createTaskMeta(projDir, 'TASK-A');
        const planFile = createPlanFile(tmpDir, ['TASK-A'], [['TASK-A']]);
        await harnessCommand({ plan: planFile, dryRun: true, skipHarnessGate: true }, tmpDir);
        const calls = logSpy.mock.calls.map((c) => c[0]).join('\n');
...
describe('harnessCommand: execution', () => {
    ...
    beforeEach(() => {
        ...
        errorSpy = spyOn(console, 'error').mockImplementation(() => { });
        runSpy = spyOn(AssemblyLine.prototype, 'run').mockResolvedValue(createMockSummary());
        reportSpy = spyOn(HarnessReporter.prototype, 'generateSummaryReport').mockResolvedValue();
    ...
    test('runs AssemblyLine and outputs summary', async () => {
        createTaskMeta(projDir, 'TASK-1');
        const planFile = createPlanFile(tmpDir, ['TASK-1']);
        await harnessCommand({ plan: planFile, skipHarnessGate: true }, tmpDir);
        expect(runSpy).toHaveBeenCalled();
        expect(reportSpy).toHaveBeenCalled();
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('执行摘要'));
        expect(exitSpy).not.toHaveBeenCalled();
    });
    test('handles execution error gracefully', async () => {
        ...
        expect(exitSpy).toHaveBeenCalledWith(1);
        expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('执行失败'), expect.any(String));
    });
    test('outputs JSON when json flag is set', async () => {
        createTaskMeta(projDir, 'TASK-1');
        const planFile = createPlanFile(tmpDir, ['TASK-1']);
        await harnessCommand({ plan: planFile, json: 'true', skipHarnessGate: true }, tmpDir);
...
describe('harnessCommand: continue mode', () => {
    ...
    test('loads saved state and resumes', async () => {
        ...
        await harnessCommand({ plan: planFile, continue: true, skipHarnessGate: true }, tmpDir);
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('从中断处继续'));
    ...
    test('starts fresh when no saved state exists', async () => {
        ...
        const planFile = createPlanFile(tmpDir, ['TASK-1']);
        await harnessCommand({ plan: planFile, continue: true, skipHarnessGate: true }, tmpDir);
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('没有找到之前的执行状态'));
        expect(runSpy).toHaveBeenCalled();
        const runArg = runSpy.mock.calls[0][0];
...
describe('harnessCommand: quality gate', () => {
    ...
    test('skips quality gate with --skip-quality-gate (deprecated)', async () => {
        createTaskMeta(projDir, 'TASK-1');
        const planFile = createPlanFile(tmpDir, ['TASK-1']);
        await harnessCommand({ plan: planFile, skipQualityGate: true }, tmpDir);
        expect(exitSpy).not.toHaveBeenCalled();
    });
    test('displays quality threshold from --require-quality', async () => {
        createTaskMeta(projDir, 'TASK-1');
        const planFile = createPlanFile(tmpDir, ['TASK-1']);
        await harnessCommand({ plan: planFile, requireQuality: '80', skipHarnessGate: true }, tmpDir);
        expect(exitSpy).not.toHaveBeenCalled();
    });
    test('quality gate runs when not skipped', async () => {
        ...
        try {
            await harnessCommand({ plan: planFile }, tmpDir);
        }
        catch { }
...
describe('harnessCommand: task filtering', () => {
    ...
    test('exits when all tasks are in terminal status', async () => {
        ...
        expect(exitSpy).toHaveBeenCalledWith(1);
        expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('没有可执行的任务'));
    });
    ...
    test('logs filtered count when tasks are removed', async () => {
        ...
        const planFile = createPlanFile(tmpDir, ['TASK-1', 'TASK-2']);
        await harnessCommand({ plan: planFile, dryRun: true, skipHarnessGate: true }, tmpDir);
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('已过滤'));
    });
});
