...
import { readAnalyzeConfig, matchesIgnorePattern, isValidISOTimestamp, validateHistoryEntry, validateRequirementHistoryEntry, validateTaskIdFormat, isValidStatusValue, isValidTypeValue, isValidPriorityValue, extractKeywordsFromCriteria, extractAcceptanceCriteriaFromDescription, inferVerificationMethod, calculateHealthScore, applySchemaMigrations, getPendingMigrations, calculateReopenStats, } from '../commands/analyze';
import { evaluateRelatedFiles, evaluateDescription, evaluateCheckpoints, evaluateSolution, evaluateLayerOrdering, extractFileRefsForLayer, } from '../utils/quality-gate';
// ============== evaluateDescription ==============
        ...
        const checkpoints = [
            {
                ...
                status: 'pending',
                verification: { method: 'unit_test', evidencePath: 'src/utils/helper.ts' },
            },
        ];
...
describe('calculateReopenStats', () => {
    test('returns zero stats for tasks with no reopens', () => {
        const tasks = [
            { id: 'T1', title: 'Task 1', reopenCount: 0, history: [] },
        ];
    ...
    test('counts tasks with reopenCount > 0', () => {
        const tasks = [
            { id: 'T1', title: 'Task 1', reopenCount: 2, history: [] },
            { id: 'T2', title: 'Task 2', reopenCount: 1, history: [] },
        ];
        const stats = calculateReopenStats(tasks);
