...
import { filterLowQualityCheckpoints, generateCheckpointId, inferVerificationFromDescription, inferCheckpointCategory, generateFallbackVerification, parseCheckpointsWithIds, syncCheckpointsToMeta, updateCheckpointStatus, getCheckpointDetail, listCheckpoints, findCheckpointIdByDescription, parseTextCheckpoints, convertParsedCheckpointsToMetadata, } from '../utils/checkpoint.js';
...
describe('inferVerificationFromDescription', () => {
    ...
    it('uses task title for test pattern extraction', () => {
        const task = { title: 'Implement auth-service feature' };
...
describe('generateFallbackVerification', () => {
    ...
    it('deduplicates file paths', () => {
        const result = generateFallbackVerification('确认 src/a.ts 和 src/a.ts 和 src/b.ts 存在');
...
describe('parseCheckpointsWithIds', () => {
    ...
    it('parses unchecked checkpoints from markdown', () => {
        ...
        readFileSyncSpy = spyOn(fs, 'readFileSync').mockReturnValue('# Task Checkpoints\n\n- [ ] 验证登录功能\n- [ ] 确认数据持久化\n');
        readTaskMetaSpy.mockReturnValue({ id: 'TASK-1', checkpoints: [] });
        const result = parseCheckpointsWithIds('TASK-1');
    ...
    it('parses checked checkpoints', () => {
        existsSyncSpy = spyOn(fs, 'existsSync').mockReturnValue(true);
        readFileSyncSpy = spyOn(fs, 'readFileSync').mockReturnValue('- [x] 已完成的功能\n- [X] 另一个完成项\n- [ ] 未完成项\n');
        readTaskMetaSpy.mockReturnValue({ id: 'TASK-1', checkpoints: [] });
        const result = parseCheckpointsWithIds('TASK-1');
    ...
    it('generates IDs when no existing metadata', () => {
        existsSyncSpy = spyOn(fs, 'existsSync').mockReturnValue(true);
        readFileSyncSpy = spyOn(fs, 'readFileSync').mockReturnValue('- [ ] 验证功能\n');
    ...
    it('matches existing checkpoint IDs by description', () => {
        existsSyncSpy = spyOn(fs, 'existsSync').mockReturnValue(true);
        readFileSyncSpy = spyOn(fs, 'readFileSync').mockReturnValue('- [ ] 验证登录功能\n');
    ...
    it('records correct line indices', () => {
        existsSyncSpy = spyOn(fs, 'existsSync').mockReturnValue(true);
        readFileSyncSpy = spyOn(fs, 'readFileSync').mockReturnValue('# Header\n\n- [ ] First\n- [ ] Second\n');
        readTaskMetaSpy.mockReturnValue(null);
        const result = parseCheckpointsWithIds('TASK-1');
...
describe('syncCheckpointsToMeta', () => {
    ...
    it('syncs checkpoint array directly to meta', () => {
        const task = { id: 'TASK-1', checkpoints: [] };
...
describe('updateCheckpointStatus', () => {
    ...
    it('throws if task does not exist', () => {
        readTaskMetaSpy.mockReturnValue(null);
        expect(() => updateCheckpointStatus('TASK-NONEXIST', 'CP-001', 'completed')).toThrow('不存在');
    });
    it('throws if checkpoint does not exist', () => {
        const task = {
            ...
            type: 'feature',
            priority: 'P2',
            checkpoints: [{ id: 'CP-001', description: '验证功能', status: 'pending', createdAt: '2026-01-01T00:00:00.000Z', updatedAt: '2026-01-01T00:00:00.000Z' }],
        };
        // Provide checkpoint.md so syncCheckpointsToMeta preserves the checkpoint
        existsSyncSpy.mockReturnValue(true);
        readFileSyncSpy.mockReturnValue('- [ ] 验证功能\n');
        readTaskMetaSpy.mockReturnValue(task);
        expect(() => updateCheckpointStatus('TASK-1', 'CP-NONEXIST', 'completed')).toThrow('不存在');
    });
...
describe('getCheckpointDetail', () => {
    ...
    it('returns checkpoint by id', () => {
        ...
        existsSyncSpy.mockReturnValue(true);
        readTaskMetaSpy.mockReturnValue({ id: 'TASK-1', title: 'Test', type: 'feature', priority: 'P2', checkpoints: [cp] });
...
describe('listCheckpoints', () => {
    ...
    it('returns empty array if task has no checkpoints', () => {
        readTaskMetaSpy.mockReturnValue({ id: 'TASK-1', title: 'Test', type: 'feature', priority: 'P2', checkpoints: [] });
    ...
    it('returns all checkpoints for a task', () => {
        ...
        readTaskMetaSpy.mockReturnValue({ id: 'TASK-1', title: 'Test', type: 'feature', priority: 'P2', checkpoints: cps });
