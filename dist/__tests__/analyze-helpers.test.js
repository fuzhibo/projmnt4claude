...
import { normalizeStatus, normalizePriority, } from '../types/task';
import { matchesIgnorePattern, isValidISOTimestamp, validateHistoryEntry, validateRequirementHistoryEntry, validateTaskIdFormat, isValidStatusValue, isValidTypeValue, isValidPriorityValue, calculateReopenStats, calculateHealthScore, extractKeywordsFromCriteria, inferVerificationMethod, extractAcceptanceCriteriaFromDescription, getPendingMigrations, applySchemaMigrations, } from '../commands/analyze';
import { evaluateRelatedFiles, evaluateDescription, evaluateCheckpoints, evaluateSolution, extractFileRefsForLayer, calculateContentQuality, evaluateLayerOrdering, } from '../utils/quality-gate';
// ============== normalizeStatus ==============
...
describe('calculateHealthScore', () => {
    ...
    test('returns 100 for empty project', () => {
        const result = calculateHealthScore(makeResult({ total: 0 }));
        expect(result).toBe(100);
    });
    test('deducts for stale tasks', () => {
        const result = calculateHealthScore(makeResult({ stale: 3 }));
        expect(result).toBeLessThan(100);
        expect(result).toBe(85); // 100 - 3*5
    });
    test('deducts for cycle dependencies', () => {
        const result = calculateHealthScore(makeResult({ cycle: 2 }));
        expect(result).toBe(70); // 100 - 2*15
    });
    test('score clamped to 0', () => {
        const result = calculateHealthScore(makeResult({ stale: 30 }));
...
describe('getPendingMigrations', () => {
    ...
    test('returns empty for current version', () => {
        const currentVersion = getPendingMigrations(0).reduce((max, m) => Math.max(max, m.version), 0);
...
describe('calculateContentQuality', () => {
    ...
    test('aggregates scores with correct weighting (structural only)', async () => {
        ...
        const expected = Math.round(result.descriptionScore * 0.35 +
...
describe('evaluateLayerOrdering', () => {
    test('returns 100 for fewer than 2 checkpoints', () => {
        const result = evaluateLayerOrdering([{ id: 'CP1', description: 'Do something', status: 'pending' }], 'description');
        expect(result.score).toBe(100);
