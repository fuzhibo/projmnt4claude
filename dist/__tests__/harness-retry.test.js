...
function createTestConfig(cwd, maxRetries = 3) {
    return {
        maxRetries,
        timeout: 60,
        parallel: 1,
...
function createReviewVerdict(overrides = {}) {
    return {
        taskId: 'TASK-test',
        result: 'NOPASS',
        reason: 'test reason',
...
describe('RetryHandler: formatRetryStatus', () => {
    ...
    test('shows 1 remaining when attempts = maxRetries - 1', () => {
        const counter = new Map([['TASK-1', 2]]);
        const status = handler.formatRetryStatus('TASK-1', counter);
