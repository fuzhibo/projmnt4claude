...
import { checkReportStatusConsistency, checkCheckpointConsistency, checkMissingPipelineEvidence, } from '../commands/analyze';
// ============== 辅助函数 ==============
...
describe('checkMissingPipelineEvidence', () => {
    ...
    test('returns null for non-pipeline-intermediate statuses', () => {
        for (const status of ['open', 'in_progress', 'resolved', 'closed', 'abandoned', 'failed']) {
            const task = createTask({ status });
...
describe('Fix actions for status inference issues', () => {
    ...
    beforeEach(() => {
        ...
        fs.writeFileSync(path.join(tmpDir, '.projmnt4claude', 'config.json'), JSON.stringify({ analyze: { autoGenerateCheckpoints: true } }), 'utf-8');
    });
    afterEach(() => {
        fs.rmSync(tmpDir, { recursive: true, force: true });
    });
