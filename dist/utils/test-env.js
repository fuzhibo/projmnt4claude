 ...
 * 生成唯一临时目录名
 */
function generateTempDirName(prefix) {
 ...
 * 创建 path 模块的 mock
 */
async function createPathMocks(state) {
 ...
 * ```
 */
export async function createIsolatedTestEnv(options = {}) {
 ...
 * ```
 */
export function resetTestEnv(env) {
    // 参数存在性验证
    if (env === null || env === undefined) {
        throw new TypeError('resetTestEnv: 参数 env 不能为 null 或 undefined。' +
            '请确保传入有效的 IsolatedTestEnv 对象（通过 createIsolatedTestEnv 创建）。');
    }
    // 参数类型验证
    if (typeof env !== 'object') {
        throw new TypeError(`resetTestEnv: 参数 env 必须是对象类型，但收到 ${typeof env}。` +
            '请确保传入有效的 IsolatedTestEnv 对象。');
    }
    // 必要属性验证
    if (typeof env.reset !== 'function') {
        throw new TypeError('resetTestEnv: 参数 env 必须包含 reset 方法。' +
 ...
 * 创建任务目录并写入元数据文件
 */
export function createTaskDir(tasksDir, taskId, meta = {}) {
    ...
    fs.writeFileSync(path.join(taskDir, 'meta.json'), JSON.stringify(fullMeta, null, 2), 'utf-8');
    return taskDir;
}
/**
 * 读取任务元数据文件
 */
export function readTaskMeta(tasksDir, taskId) {
 ...
 * 写入任务元数据文件
 */
export function writeTaskMeta(tasksDir, taskId, meta) {
    const taskDir = path.join(tasksDir, taskId);
    ensureDir(taskDir);
    fs.writeFileSync(path.join(taskDir, 'meta.json'), JSON.stringify(meta, null, 2), 'utf-8');
}
/**
 * 检查任务是否存在
 */
export function taskExists(tasksDir, taskId) {
    return fs.existsSync(path.join(tasksDir, taskId, 'meta.json'));
}
/**
 * 获取所有任务ID列表
 */
export function getAllTaskIds(tasksDir) {
 ...
 * 创建归档任务（用于测试归档功能）
 */
export function createArchivedTask(projectDir, taskId, meta = {}) {
    ...
    const fullMeta = { ...defaultMeta, ...meta };
    fs.writeFileSync(path.join(archiveDir, 'meta.json'), JSON.stringify(fullMeta, null, 2), 'utf-8');
    return archiveDir;
}
// ============================================================
// 批量测试辅助
 ...
 * 批量创建测试任务
 */
export function createTestTasks(tasksDir, count, baseMeta = {}) {
    const taskIds = [];
 ...
 * 创建测试任务依赖关系
 */
export function createTaskDependency(tasksDir, parentId, childId) {
    const parentMeta = readTaskMeta(tasksDir, parentId);
    if (parentMeta) {
        const deps = parentMeta.dependencies || [];
