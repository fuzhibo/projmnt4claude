 ...
 * 检查输出是否为合法 JSON 字符串
 */
export const jsonParseableRule = {
    id: 'json-parseable',
 ...
 */
export const nonEmptyOutputRule = {
    id: 'non-empty-output',
    description: '输出不能为空',
    severity: 'error',
    check: (output) => {
 ...
 */
export class JsonFeedbackTemplate {
    truncationLimit;
 ...
 */
export class MarkdownFeedbackTemplate {
    truncationLimit;
    constructor(truncationLimit = 4000) {
        this.truncationLimit = truncationLimit;
    }
 ...
 * 4. 执行"调用 → 验证 → 反馈 → 重试"循环
 */
export class FeedbackConstraintEngineImpl {
    ruleSets = [];
    template;
    retryCount = 0;
    ...
    validate(output) {
        ...
        if (violations.length > 0) {
            logger.debug(`[FeedbackConstraintEngine] 验证发现 ${violations.length} 个违规项`);
        }
        return violations;
    }
    /**
    ...
    shouldRetry(violations) {
        ...
        for (const ruleSet of this.ruleSets) {
            const errorViolationsInSet = violations.filter((v) => v.severity === 'error' &&
                ruleSet.rules.some((r) => r.id === v.ruleId));
            if (errorViolationsInSet.length > 0) {
                if (this.retryCount >= ruleSet.maxRetriesOnError) {
                    logger.debug(`[FeedbackConstraintEngine] 规则集 "${ruleSet.name}" 已达到最大重试次数 (${ruleSet.maxRetriesOnError})`);
                    return false;
                }
            }
        }
    ...
    options) {
        ...
        while (true) {
            ...
            // 无违规，通过
            if (violations.length === 0) {
                logger.debug(`[FeedbackConstraintEngine] 验证通过 (重试 ${this.retryCount} 次)`);
                return {
                    result: lastResult,
                    violations: [],
            ...
            if (!this.shouldRetry(violations)) {
                const hasErrors = violations.some((v) => v.severity === 'error');
                logger.debug(`[FeedbackConstraintEngine] 验证未通过，不再重试 (错误: ${hasErrors}, 重试: ${this.retryCount})`);
                return {
                    result: lastResult,
                    violations,
                    retries: this.retryCount,
 ...
 */
export function createJsonFeedbackEngine(additionalRules = [], maxRetriesOnError = 2) {
 ...
 */
export function createMarkdownFeedbackEngine(rules = [], maxRetriesOnError = 2) {
    const engine = new FeedbackConstraintEngineImpl(new MarkdownFeedbackTemplate());
    engine.addRuleSet({
 ...
 */
export function createSessionAwareEngine(outputType = 'json', rules = [], maxRetriesOnError = 2) {
