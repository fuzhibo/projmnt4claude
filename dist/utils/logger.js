...
export class Logger {
    ...
    generateBugReport(recentCount = 100) {
        ...
        // 生成压缩附件
        const archivePath = this.compressLogs(path.join(getLogsDir(this.cwd), '..', `bug-report-${timestamp}.tar.gz`));
    ...
    analyzeUsage() {
        ...
        let totalWarnings = 0;
        // 跟踪命令开始/结束用于计算耗时
        const commandStarts = {}; // command -> timestamp
 ...
 */
export function createLogger(command, cwd) {
