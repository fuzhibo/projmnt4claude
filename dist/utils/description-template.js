 ...
 * 生成简单格式的描述
 */
function generateSimpleDescription(data) {
    const parts = [];
    parts.push(`## 问题描述\n${data.problem}`);
    if (data.rootCause) {
        parts.push(`\n## 根因分析\n${data.rootCause}`);
 ...
 */
export function generateStructuredDescription(data, templateType = 'simple') {
 ...
 */
export function extractStructuredInfo(input) {
 ...
 * 提取根因分析
 */
function extractRootCause(input) {
    // 匹配 "根因分析" 或 "原因" 等模式
 ...
 * 提取解决方案
 */
function extractSolution(input) {
    // 匹配 "解决方案" 或 "方案" 等模式
 ...
 * 提取检查点
 */
function extractCheckpoints(input) {
    const checkpoints = [];
    // 匹配 "检查点" 或 "验收标准" 等模式
 ...
 * 提取相关文件
 */
function extractRelatedFiles(input) {
    const files = [];
    // 匹配 "相关文件" 模式
 ...
 * 从解决方案部分提取具体检查点
 */
function extractSolutionCheckpoints(description) {
    const checkpoints = [];
    // 匹配解决方案/方案部分
 ...
 * 判断文本是否包含具体可识别的术语
 */
function containsSpecificTerms(text) {
    // 包含代码标识符（3+字符的英文词）
    if (/[a-zA-Z_][a-zA-Z0-9_]{2,}/.test(text))
        return true;
    // 包含中文代码相关术语
    if (/(?:函数|方法|类|模块|接口|API|配置|文件|变量|组件|服务|插件|命令)/.test(text))
        return true;
    // 包含文件路径
    if (/\.{0,2}\/[\w/-]+\.[a-z]+/.test(text))
        return true;
    return false;
}
/**
 ...
 * 仅在文本过于模糊时通过实体提取重构。
 */
function convertStepToCheckpoint(step) {
    if (step.length < 5)
        return null;
    // 过滤掉泛化的流程阶段（如"需求分析"、"核心功能实现"等）
    if (isGenericCheckpoint(step))
        return null;
    // 包含具体术语时直接保留
    if (containsSpecificTerms(step))
        return step;
...
function extractEntities(text) {
    ...
    for (const match of funcMatches) {
        if (match[1])
            entities.push({ type: 'function', name: match[1], action: 'call' });
    }
    ...
    for (const match of fileMatches) {
        if (match[1])
            entities.push({ type: 'file', name: match[1], action: 'modify' });
    }
    // 匹配类名 (PascalCase)
    ...
    for (const match of moduleMatches) {
        if (match[1])
            entities.push({ type: 'module', name: match[1], action: 'create' });
    }
    return entities;
}
/**
 * 格式化检查点
 */
function formatCheckpoint(entity, originalStep) {
    const actionVerbs = {
        export: '导出',
        import: '导入',
        call: '调用',
 ...
 * 从问题描述部分提取检查点
 */
function extractProblemCheckpoints(description) {
    const checkpoints = [];
    // 匹配问题描述部分
    ...
    for (const match of errorMatches) {
        if (match[1])
            checkpoints.push(`修复错误: ${match[1].trim()}`);
    }
    return checkpoints;
}
/**
 * 从根因分析部分提取检查点
 */
function extractRootCauseCheckpoints(description) {
    const checkpoints = [];
    // 匹配根因分析部分
 ...
 */
function extractActionBasedCheckpoints(description) {
    const checkpoints = [];
    // 匹配编号列表中的具体动作项
 ...
 * 基于任务描述内容生成具体的可验证条件
 */
export function inferCheckpointsFromDescription(description, type) {
    const checkpoints = [];
 ...
 */
function generateEntityFallbackCheckpoints(entities, type, description) {
    const checkpoints = [];
 ...
 * 绝不生成泛化的流程阶段检查点。
 */
function generateDescriptionBasedCheckpoints(description, type) {
    const checkpoints = [];
    // 策略 1: 提取 "动词+目标" 模式
    const actionTargetPatterns = [
 ...
 * 判断是否为泛化的流程阶段检查点
 */
function isGenericCheckpoint(text) {
 ...
 * 从标题中提取有意义的关键词组合
 */
function generateTitleKeywordCheckpoints(description, type) {
    const checkpoints = [];
 ...
 * 推断相关文件
 */
export function inferRelatedFiles(description, type) {
    const files = [];
    const lowerDesc = description.toLowerCase();
    // 基于关键词推断文件
