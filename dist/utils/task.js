...
export const VALID_TRANSITIONS = {
    ...
    wait_qa: ['wait_evaluation', 'in_progress', 'failed', 'closed', 'abandoned'],
    wait_evaluation: ['resolved', 'in_progress', 'failed', 'closed', 'abandoned'],
 ...
 * 获取任务目录路径
 */
export function getTaskDir(taskId, cwd = process.cwd()) {
    return path.join(getTasksDir(cwd), taskId);
}
/**
 * 获取任务元数据文件路径
 */
export function getTaskMetaPath(taskId, cwd = process.cwd()) {
    return path.join(getTaskDir(taskId, cwd), 'meta.json');
}
/**
 * 读取任务元数据
 */
export function readTaskMeta(taskId, cwd = process.cwd()) {
 ...
 * 写入任务元数据（自动记录历史)
 */
export function writeTaskMeta(task, cwd = process.cwd()) {
    ...
    if (oldTask) {
        // 比较并记录字段变更
        const fields = ['title', 'description', 'priority', 'status', 'recommendedRole', 'branch', 'dependencies', 'type', 'subtaskIds', 'parentId', 'checkpoints', 'schemaVersion'];
        for (const field of fields) {
            ...
            if (field === 'dependencies') {
                const oldDepsArray = (oldValue || []);
                const newDepsArray = (newValue || []);
 ...
 * 获取所有任务ID
 */
export function getAllTaskIds(cwd = process.cwd()) {
    if (!isInitialized(cwd)) {
        return [];
    }
 ...
 * 获取所有任务
 */
export function getAllTasks(cwd = process.cwd(), includeArchived = false) {
 ...
 */
export function generateNewTaskId(cwd = process.cwd(), type = 'feature', priority = 'P2', title = 'task') {
 ...
 * 检查任务是否存在
 */
export function taskExists(taskId, cwd = process.cwd()) {
 ...
 */
export function generateSubtaskId(parentId, cwd = process.cwd()) {
 ...
 * 将子任务关联到父任务
 */
export function addSubtaskToParent(parentId, subtaskId, cwd = process.cwd()) {
    const parentTask = readTaskMeta(parentId, cwd);
    if (!parentTask) {
        throw new Error(`父任务 ${parentId} 不存在`);
    }
 ...
 * 获取所有子任务
 */
export function getSubtasks(parentId, cwd = process.cwd()) {
    const parentTask = readTaskMeta(parentId, cwd);
    if (!parentTask) {
        return [];
    }
 ...
 */
export function parseParentFromSubtaskId(subtaskId) {
 ...
 * 获取父任务
 */
export function getParentTask(taskId, cwd = process.cwd()) {
 ...
 */
function shouldRecordHistory(history, field, oldValue, newValue) {
    // 新值与旧值相同，不需要记录
 ...
 */
export function updateTaskStatus(taskId, status, cwd = process.cwd(), reason, transitionNote, options) {
    const task = readTaskMeta(taskId, cwd);
    if (!task) {
 ...
 * 基于检查点状态计算验证结果
 */
export function buildTaskVerification(task) {
    const now = new Date().toISOString();
 ...
 * 直接修改 meta.json，不依赖 AI 上下文
 */
export function assignRole(taskId, role, cwd = process.cwd()) {
    const task = readTaskMeta(taskId, cwd);
    if (!task) {
        throw new Error(`任务 '${taskId}' 不存在`);
    }
 ...
 * 直接修改 meta.json，不依赖 AI 上下文
 */
export function incrementReopenCount(taskId, reason, cwd = process.cwd()) {
    const task = readTaskMeta(taskId, cwd);
    if (!task) {
        throw new Error(`任务 '${taskId}' 不存在`);
    }
 ...
 * 直接修改 meta.json，不依赖 AI 上下文
 */
export function recordExecutionStats(taskId, stats, cwd = process.cwd()) {
    const task = readTaskMeta(taskId, cwd);
    if (!task) {
        throw new Error(`任务 '${taskId}' 不存在`);
    }
 ...
 * @returns 新的任务 ID
 */
export function renameTask(oldTaskId, newTaskId, cwd = process.cwd()) {
    // 1. 验证旧任务存在
 ...
 */
function updateTaskReferences(oldId, newId, cwd) {
    if (oldId === newId)
        return;
    const allIds = getAllTaskIds(cwd);
    for (const tid of allIds) {
        if (tid === oldId || tid === newId)
            continue; // 跳过自身
