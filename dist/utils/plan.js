 ...
 * - 纯文本（每行一个任务ID）
 */
export function parsePlanOutput(input) {
    const result = {
        tasks: [],
        batches: [],
        metadata: {},
    ...
    try {
        ...
        if (Array.isArray(parsed.tasks)) {
            result.tasks = parsed.tasks.filter((t) => typeof t === 'string');
        }
        // 提取 batches
        if (Array.isArray(parsed.batches)) {
            result.batches = parsed.batches
                .filter((b) => Array.isArray(b))
                .map((b) => b.filter((t) => typeof t === 'string'));
        }
 ...
 */
export function extractTaskIdsFromPlan(plan) {
    if (!plan) {
        return [];
    }
    // 优先使用 batches（按顺序展平）
    ...
    if (Array.isArray(plan.tasks)) {
        return plan.tasks.filter((t) => typeof t === 'string' && t.length > 0);
    }
    return [];
}
/**
 ...
 */
export function calculateBatchSize(totalTasks, maxBatchSize = 5) {
 ...
 */
export function detectTaskRelations(tasks) {
 ...
 */
const EXECUTABLE_STATUS_PRIORITY = {
 ...
 * 读取执行计划
 */
export function readPlan(cwd = process.cwd()) {
 ...
 * 写入执行计划
 */
export function writePlan(plan, cwd = process.cwd()) {
    const planPath = getPlanPath(cwd);
 ...
 * 获取或创建计划
 */
export function getOrCreatePlan(cwd = process.cwd()) {
 ...
 * 添加任务到计划
 */
export function addTaskToPlan(taskId, afterId, cwd = process.cwd()) {
 ...
 * 从计划移除任务
 */
export function removeTaskFromPlan(taskId, cwd = process.cwd()) {
    const plan = readPlan(cwd);
    if (!plan) {
        return false;
    }
 ...
 * 清空计划
 */
export function clearPlan(cwd = process.cwd()) {
 ...
 * 检查任务依赖是否全部完成
 */
export function areDependenciesCompleted(taskId, cwd = process.cwd()) {
 ...
 * 检查子任务的父任务是否已完成
 */
export function isParentTaskCompleted(taskId, cwd = process.cwd()) {
    const task = readTaskMeta(taskId, cwd);
 ...
 * - 跳过父任务已完成的子任务
 */
export function getExecutableTasks(cwd = process.cwd(), includeSubtasks = false) {
 ...
 * @returns 缺失子任务告警列表
 */
export function detectMissingSubtasks(cwd = process.cwd()) {
