...
function startsWithVerb(text) {
    ...
    for (const verb of CHINESE_VERBS) {
        if (firstWord.startsWith(verb))
            return true;
    }
    // 英文：精确匹配或常见时态变形
    if (ENGLISH_VERBS.has(firstWord))
        return true;
    // 处理 -ed / -ing / -es / -s 变形
    ...
    for (const candidate of stemCandidates) {
        if (ENGLISH_VERBS.has(candidate))
            return true;
    }
    return false;
}
 ...
 */
export const checkpointNoDuplicate = {
    ...
    check: (output) => {
        ...
        if (checkpoints.length <= 1)
            return null;
        const duplicates = [];
        const seen = new Map(); // index -> normalized
 ...
 * 过滤纯文件路径格式的检查点描述
 */
export const checkpointNoFilePath = {
 ...
 * 控制检查点数量在合理范围内
 */
export const checkpointCountControl = {
    ...
    check: (output) => {
        ...
        if (count <= 8)
            return null;
        const severity = count > 15 ? 'error' : 'warning';
        return {
 ...
 * 检查点描述应以动词开头
 */
export const checkpointVerbPrefix = {
 ...
 * 每条检查点描述至少 10 个字符
 */
export const checkpointMinLength = {
 ...
 */
function extractCheckpointDescriptions(output) {
    ...
    if (output !== null && typeof output === 'object') {
        ...
        for (const key of candidates) {
            if (Array.isArray(obj[key])) {
                return obj[key].filter((item) => typeof item === 'string');
            }
        }
 ...
 */
function extractCheckpointObjects(output) {
    if (Array.isArray(output)) {
        return output.filter((item) => item !== null && typeof item === 'object' && !Array.isArray(item));
    }
    if (typeof output === 'string') {
        try {
    ...
    if (output !== null && typeof output === 'object') {
        ...
        if (Array.isArray(obj.checkpoints)) {
            return obj.checkpoints.filter((item) => item !== null && typeof item === 'object' && !Array.isArray(item));
        }
    }
    return [];
}
...
/** 允许的检查点描述前缀（验证类别标识） */
export const VALID_CHECKPOINT_PREFIXES = [
    '[ai review]',
    '[ai qa]',
    '[human qa]',
    '[script]',
 ...
 */
export function getCheckpointPhase(description) {
 ...
 */
export function inferCheckpointAttributesFromPrefix(description) {
    if (!description || typeof description !== 'string') {
        return {};
    }
    const trimmed = description.trim().toLowerCase();
 ...
 */
export function inferCheckpointPrefix(description) {
    if (!description || typeof description !== 'string') {
        return '[ai review]';
    }
 ...
 */
export const checkpointRequiredPrefix = {
 ...
 */
export const checkpointHasVerificationCommands = {
    ...
    check: (output) => {
        ...
        for (const cp of checkpoints) {
            ...
            if (!method || !METHODS_REQUIRING_COMMANDS.includes(method))
                continue;
 ...
 * - 截断/不完整的字符串
 */
function detectStringFormatIssues(value) {
    const issues = [];
    // 检测未转义的控制字符
...
export const metaJsonValid = {
    ...
    check: (output) => {
        ...
        if (!Array.isArray(obj['history']))
            errors.push('history');
        if (!Array.isArray(obj['checkpoints']))
            errors.push('checkpoints');
        if (!Array.isArray(obj['subtaskIds']))
            errors.push('subtaskIds');
        if (!Array.isArray(obj['discussionTopics']))
            errors.push('discussionTopics');
        if (!Array.isArray(obj['fileWarnings']))
            errors.push('fileWarnings');
        if (!Array.isArray(obj['allowedTools']))
            errors.push('allowedTools');
        ...
        const stringFields = ['id', 'title', 'type', 'status', 'description'];
...
// ============================================================
/** 所有检查点质量验证规则 */
export const checkpointValidationRules = [
