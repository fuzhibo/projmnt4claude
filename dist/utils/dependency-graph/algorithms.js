 ...
 * 返回所有环的完整路径
 */
export function detectCyclesDFS(adjacency) {
    const cycles = [];
 ...
 */
export function wouldCreateCycleBFS(from, to, reverseAdj) {
 ...
 * 返回每个分量的信息
 */
export function findComponentsUnionFind(adjacency, nodes) {
 ...
 */
function computeComponentDepth(rootId, adjacency, members) {
 ...
 * 返回分层结构，同层可并行执行
 */
export function topologicalSortDFS(adjacency, nodeIds) {
 ...
 */
function buildSimpleAdjacency(adjacency, nodeIds) {
    const result = new Map();
    for (const [from, deps] of adjacency) {
        if (nodeIds && !nodeIds.has(from))
            continue;
 ...
 */
export function computeTransitiveClosure(adjacency) {
    ...
    for (const node of allNodes) {
        if (!simpleAdj.has(node))
            simpleAdj.set(node, new Set());
    }
    for (const [from, deps] of adjacency) {
 ...
 */
export function breakCycle(cyclePath, edges) {
 ...
 * 若到达不同的根节点，则标记为桥接节点
 */
export function detectBridgeNodes(reverseAdj, roots) {
    const bridges = [];
 ...
 */
export function computeDepths(adjacency, roots) {
