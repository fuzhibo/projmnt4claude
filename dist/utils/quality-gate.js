...
import { checkpointRequiredPrefix, checkpointHasVerificationCommands, metaJsonValid, VALID_CHECKPOINT_PREFIXES, getCheckpointPhase, } from './validation-rules/checkpoint-rules.js';
import { AIMetadataAssistant, classifyFileToLayer, groupFilesByLayer } from './ai-metadata.js';
 ...
 */
export function extractFilePaths(text, options) {
    const files = [];
    const seen = new Set();
 ...
 * 评估关联文件
 */
export function evaluateRelatedFiles(description, checkpoints) {
    const deductions = [];
    let score = 100;
    // 从描述中检测文件引用
 ...
 */
export function extractAffectedFiles(task) {
    const files = [];
    const seen = new Set();
 ...
 * 在流水线阶段流转时做兜底的基础字段检查。
 */
export function validateBasicFields(task) {
    const errors = [];
    const missingFields = [];
    // 检查 ID
 ...
 * 确保引用的文件在项目目录中真实存在。
 */
export function validateFilesExist(task, cwd = process.cwd()) {
 ...
 * 推断变更范围大小
 */
function inferChangeSize(task, affectedFiles) {
    // 基于文件数量推断
    ...
    // 基于检查点数量推断
    if (task.checkpoints && task.checkpoints.length >= 5)
        return 'large';
    if (task.checkpoints && task.checkpoints.length >= 3)
        return 'medium';
    return 'small';
}
/**
 * 检查是否需要解决方案确认
 */
function requiresSolutionConfirmation(task) {
    const description = task.description || '';
    // 检查是否有解决方案部分
 ...
 * 生成质量门禁建议
 */
function generateSuggestions(task, score, affectedFiles, changeSize, requiresConfirmation) {
    const suggestions = [];
    // 基于扣分项生成建议
 ...
 * 检查单个任务的质量门禁
 */
export async function checkQualityGate(taskId, config = DEFAULT_QUALITY_GATE_CONFIG, cwd = process.cwd()) {
    ...
    // 生成建议
    const suggestions = generateSuggestions(task, score, affectedFiles, changeSize, needsConfirmation);
    // 将检查点验证违规转化为建议
 ...
 */
export function validateCheckpoints(task) {
    const violations = [];
    // 获取检查点策略（显式声明 > 自动推断）
    const policy = task.checkpointPolicy ?? inferCheckpointPolicy(task.type, task.priority);
 ...
 * 评估描述完整度
 */
export function evaluateDescription(description) {
    const deductions = [];
    let score = 100;
 ...
 * 评估检查点质量
 */
export function evaluateCheckpoints(checkpoints, policy) {
    const deductions = [];
    let score = 100;
 ...
 * 评估解决方案
 */
export function evaluateSolution(description) {
    const deductions = [];
    let score = 100;
    if (!description || description.trim().length === 0) {
 ...
 * 从文本中提取文件引用路径（用于层级分析）
 */
export function extractFileRefsForLayer(text) {
    const files = [];
    const seen = new Set();
    const patterns = [
 ...
 */
export function evaluateLayerOrdering(checkpoints, description) {
    const deductions = [];
    let score = 100;
    ...
    if (layersPresent.length >= 2) {
        // 检查检查点描述中是否提到了文件路径
        const cpWithFiles = checkpoints.filter(cp => filePaths.some(fp => cp.description.includes(fp)));
 ...
 */
export async function calculateContentQuality(task, aiOptions, cwd) {
    const deductions = [];
    ...
    const structuralTotal = Math.round(descriptionScore * 0.35 +
 ...
 * 批量检查任务队列的质量门禁
 */
export async function batchCheckQualityGate(taskIds, config = DEFAULT_QUALITY_GATE_CONFIG, cwd = process.cwd()) {
 ...
 * 格式化质量门禁检查结果
 */
export function formatQualityGateResult(result, options = {}) {
 ...
 * 格式化批量检查结果
 */
export function formatBatchQualityGateResult(batchResult, options = {}) {
 ...
 * 输出质量门禁完善指南
 */
export function showQualityImprovementGuide(result) {
    console.log('');
 ...
 * 确认理解解决方案（交互式）
 */
export function confirmSolutionUnderstanding(task, result) {
    // 如果不需要确认，直接通过
    if (!result.requiresConfirmation) {
