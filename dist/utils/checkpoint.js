 ...
 */
export function filterLowQualityCheckpoints(checkpoints) {
    const kept = [];
    const removed = [];
 ...
 */
export function generateCheckpointId(taskId, index, description) {
    // 从描述中提取关键词生成 slug
 ...
 */
export function parseCheckpointsWithIds(taskId, cwd = process.cwd()) {
 ...
 */
function extractTestPatterns(desc, task) {
    const patterns = [];
    ...
    // 2. 从描述中提取模块/功能名称
    const moduleKeywords = desc.match(/(?:验证|测试|检查|test|verify|check|validate)\s*([\w-]{3,})/i);
    if (moduleKeywords?.[1]) {
 ...
 */
export function generateFallbackVerification(description, task) {
    const commands = [];
    const steps = [];
 ...
 */
export function inferVerificationFromDescription(description, task) {
 ...
 * 为检查点推断验证类别
 */
export function inferCheckpointCategory(description) {
    const lowerDesc = description.toLowerCase();
    // 代码审查类
 ...
 */
export function syncCheckpointsToMeta(taskId, checkpointsOrCwd, maybeCwd) {
    // 解析参数
    ...
    const filteredCheckpoints = parsedCheckpoints.filter(cp => !filterResult.removed.includes(cp.text));
    ...
    // 合并现有元数据和新解析的检查点（已过滤）
    const mergedCheckpoints = filteredCheckpoints.map((cp, index) => {
 ...
 * 更新检查点状态
 */
export function updateCheckpointStatus(taskId, checkpointId, status, options = {}, cwd = process.cwd()) {
    const task = readTaskMeta(taskId, cwd);
    if (!task) {
        throw new Error(`任务 '${taskId}' 不存在`);
    }
 ...
 * 获取检查点详情
 */
export function getCheckpointDetail(taskId, checkpointId, cwd = process.cwd()) {
    syncCheckpointsToMeta(taskId, cwd);
    const task = readTaskMeta(taskId, cwd);
 ...
 * 列出所有检查点
 */
export function listCheckpoints(taskId, cwd = process.cwd()) {
    syncCheckpointsToMeta(taskId, cwd);
    const task = readTaskMeta(taskId, cwd);
 ...
 * 通过描述查找检查点ID
 */
export function findCheckpointIdByDescription(taskId, description, cwd = process.cwd()) {
    const checkpoints = listCheckpoints(taskId, cwd);
    const found = checkpoints.find(cp => cp.description === description ||
 ...
 * @param cwd 工作目录
 */
export function updateCheckpointMdFromArray(taskId, checkpoints, cwd = process.cwd()) {
    const checkpointPath = path.join(getTasksDir(cwd), taskId, 'checkpoint.md');
 ...
 * @returns 解析出的检查点列表
 */
export function parseTextCheckpoints(description) {
 ...
 * @returns 检查点元数据列表
 */
export function convertParsedCheckpointsToMetadata(parsedCheckpoints, task) {
    const now = new Date().toISOString();
 ...
 * @returns 是否进行了同步
 */
export function syncTextCheckpointsToMeta(taskId, cwd = process.cwd()) {
    const task = readTaskMeta(taskId, cwd);
    if (!task) {
        throw new Error(`任务 '${taskId}' 不存在`);
    }
 ...
 * @returns 修复后的检查点列表
 */
export async function fixMissingCheckpoints(taskId, options = {}, cwd = process.cwd()) {
    const task = readTaskMeta(taskId, cwd);
    if (!task) {
        throw new Error(`任务 '${taskId}' 不存在`);
    }
 ...
 * 基于任务描述和相关信息生成智能检查点
 */
async function generateCheckpointsForTask(task, options, cwd) {
    const now = new Date().toISOString();
    const checkpoints = [];
    // 从任务描述中提取验收标准
    const acceptanceCriteria = extractAcceptanceCriteria(options.taskDescription || task.description || '', options.taskTitle || task.title);
    // 基于验收标准生成检查点
 ...
 * 从任务描述中提取验收标准
 */
function extractAcceptanceCriteria(description, taskTitle) {
    const criteria = [];
 ...
 * 保留现有的状态信息，避免重复
 */
function mergeCheckpoints(existing, generated) {
    const merged = [];
    const usedExisting = new Set();
    // 首先尝试匹配新生成的检查点与现有检查点
    for (const gen of generated) {
        const match = existing.find(ex => ex.description === gen.description ||
            ex.id === gen.id);
        if (match) {
            // 保留现有检查点的状态，但更新其他信息
            merged.push({
