...
import { decomposeRequirement, shouldDecompose, formatDecomposition, validateDecompositionItem, validateDecompositionItems, } from '../requirement-decomposer';
...
非紧急的视觉优化，可以延后处理。
      `;
        ...
        test('应该限制最大分解数量', async () => {
            const content = Array.from({ length: 15 }, (_, i) => `问题 ${i + 1}: 描述问题${i + 1}的详细内容，包括具体表现和影响范围。` +
                `需要修复相关代码，添加测试用例，确保功能正常。`).join('\n\n') + '\n\n请按优先级处理以上问题。';
            const result = await decomposeRequirement(content, {
                useAI: false,
                minItems: 2,
                maxItems: 5,
