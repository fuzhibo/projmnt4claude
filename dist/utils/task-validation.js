...
/** 需要有默认值的数组字段及其默认值 */
const SCHEMA_ARRAY_FIELD_DEFAULTS = {
    dependencies: [],
    history: [],
    checkpoints: [],
    subtaskIds: [],
 ...
 */
export function validateSchemaVersionChange(oldVersion, newVersion) {
    const errors = [];
    const warnings = [];
 ...
 */
export function validateSchemaFieldDefaults(task) {
    const errors = [];
    const warnings = [];
    // 数组字段不允许为 null
 ...
 */
export function validateSchemaMigration(oldTask, newTask) {
    const errors = [];
    const warnings = [];
    // schemaVersion 单调递增
    if (oldTask) {
...
// ============================================================
/** 合法检查点状态转换映射 */
export const VALID_CHECKPOINT_TRANSITIONS = {
 ...
 */
export function validateCheckpointTransition(fromStatus, toStatus) {
 ...
 * 验证检查点 ID 是否存在于任务定义中。
 */
export function validateCheckpointId(task, checkpointId) {
 ...
 */
export function validateCheckpointOperations(oldTask, newTask) {
...
const VALID_VERIFICATION_RESULTS = ['passed', 'partial', 'failed'];
 ...
 */
export function validateVerificationBackfill(verification) {
    const errors = [];
    const warnings = [];
    // verifiedAt
    ...
    // result
    if (!VALID_VERIFICATION_RESULTS.includes(verification.result)) {
        errors.push(`verification.result "${verification.result}" 不合法，允许值: ${VALID_VERIFICATION_RESULTS.join(', ')}`);
    }
    ...
    if (verification.checkpointCompletionRate !== undefined) {
        if (typeof verification.checkpointCompletionRate !== 'number'
            ...
            errors.push(`verification.checkpointCompletionRate ${verification.checkpointCompletionRate} 不在 0-100 范围内`);
        }
    }
    // methods
 ...
 */
export function validateBackfillOperation(oldTask, newTask) {
 ...
 */
export function validateFieldUpdate(field, value) {
    const errors = [];
    const warnings = [];
    switch (field) {
        case 'priority': {
            if (typeof value !== 'string' || !VALID_PRIORITIES.includes(value)) {
                errors.push(`无效优先级 "${value}"，允许值: ${VALID_PRIORITIES.join(', ')}`);
            }
            break;
        }
        case 'type': {
            if (typeof value !== 'string' || !VALID_TYPES.includes(value)) {
                errors.push(`无效类型 "${value}"，允许值: ${VALID_TYPES.join(', ')}`);
            }
            break;
        }
        case 'title': {
 ...
 * - 不允许循环引用
 */
export function validateRelationship(taskId, updates, cwd) {
    const errors = [];
    const warnings = [];
    // 验证 parentId
 ...
 */
export function validateTaskBeforeWrite(task, cwd, oldTask = null, options = {}) {
    const { validateSchema = true, validateCheckpoint = true, validateBackfill = true, } = options;
    const allErrors = [];
    const allWarnings = [];
    // 字段验证（始终执行）
    const fields = [
    ...
    // 关系验证（始终执行）
    const relResult = validateRelationship(task.id, { parentId: task.parentId, subtaskIds: task.subtaskIds }, cwd);
 ...
 */
export function validatedWriteTaskMeta(task, cwd, options = {}) {
    const { strict = false, validateSchema = true, validateCheckpoint = true, validateBackfill = true, } = options;
    // 读取旧任务用于对比验证
...
export class TaskValidationError extends Error {
    taskId;
    validationError;
    constructor(taskId, validationError) {
