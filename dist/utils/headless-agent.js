...
import { DEFAULT_AI } from '../types/config.js';
// ============================================================
 ...
 */
export function translateOptionsToCliArgs(options) {
    const args = ['--print'];
 ...
 */
export function buildEffectiveTools(phase, cwd, task) {
    // Level 3: 代码默认值
...
export function loadAIConfig(cwd) {
    try {
        const configPath = getConfigPath(cwd);
        if (!fs.existsSync(configPath)) {
            return DEFAULT_AI;
