 ...
 * @returns 推断的依赖列表
 */
export function inferDependencies(taskOrDescription, allTasks, options) {
    const { strategy = 'all', skipTerminal = true, keywordHints = [], } = options || {};
    // 解析当前任务文件列表
    let currentTaskId;
    let currentFiles;
    ...
    if (strategy === 'all' || strategy === 'file-overlap') {
        for (const candidate of candidates) {
            ...
            const normalizedCandidate = new Set(candidateFiles.map(f => f.replace(/\/+$/, '').toLowerCase()));
            const overlap = [];
 ...
 */
export function inferDependenciesBatch(tasks) {
    const resultMap = new Map();
    if (tasks.length === 0)
        return resultMap;
    // 预计算每个任务的规范化文件路径集合
    const taskFilesMap = new Map();
    ...
    // 按创建时间升序排列，确定依赖方向
    const sortedByTime = [...tasks].sort((a, b) => a.createdAt.localeCompare(b.createdAt));
    // O(n²) 两两比较
