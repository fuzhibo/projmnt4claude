...
7. 如果无法分解（如内容过于简单），返回 {"decomposable": false, "reason": "原因", "items": []}`;
        ...
        const result = await assistant.invokeWithEngine(prompt, [], { cwd, maxRetries: 2, timeoutSeconds: 30 });
        if (!result.success) {
            return null;
        }
        ...
        return {
            ...
            items: parsed.items.map((item, index) => ({
                ...
                suggestedCheckpoints: Array.isArray(item.suggestedCheckpoints)
                    ? item.suggestedCheckpoints.filter((c) => typeof c === 'string')
                    : [],
                relatedFiles: Array.isArray(item.relatedFiles)
                    ? item.relatedFiles.filter((f) => typeof f === 'string')
 ...
 */
export async function decomposeRequirement(content, options = {}) {
    const { minItems = 2, maxItems = 10, useAI = true, cwd = process.cwd(), validateQuality = true, // 默认启用质量检查
     } = options;
    // 清理输入内容
 ...
 * 快速预检查，用于决定是否显示分解选项
 */
export function shouldDecompose(content) {
 ...
 * 格式化分解结果供显示
 */
export function formatDecomposition(decomposition) {
 ...
 * @returns 验证结果
 */
export function validateDecompositionItem(item) {
    const errors = [];
    const warnings = [];
    const { MIN_TITLE_LENGTH, MIN_PROBLEM_LENGTH, MIN_SOLUTION_LENGTH, MIN_CHECKPOINTS, VALID_PRIORITIES, } = DECOMPOSITION_CONSTRAINTS;
    // 检查标题
 ...
 */
export function validateDecompositionItems(items) {
    const validItems = [];
    const invalidItems = [];
    const allErrors = [];
 ...
 */
export function reportDecompositionFailure(reason, errors, itemTitle) {
    console.error('❌ 分解失败');
    if (itemTitle) {
 ...
 * @returns 转换后的分解项
 */
export function convertToDecomposedItem(item) {
    return {
        title: item.title,
