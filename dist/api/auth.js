 ...
 */
export async function login(request, timeout = DEFAULT_TIMEOUT) {
...
export function createAuthClient(config) {
    return {
         ...
         * 登录方法
         */
        login: (request) => login(request, config.timeout),
        /**
         * 登出方法
         */
        logout: async () => {
            try {
                await fetchWithTimeout(`${config.baseUrl}/api/auth/logout`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
