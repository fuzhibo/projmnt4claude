...
// 测试用例模板
const TEST_CASE_TEMPLATES: Record<string, string[]> = {
  'config.ts': [
...
// 生成标准化的检查点
function generateCheckpoints(moduleName: string, testFunctions: string[]): any[] {
...
// 解析任务描述中的测试函数
function parseTestFunctions(description: string): string[] {
...
// 获取模块名
function getModuleName(taskId: string): string {
...
// 更新单个任务
async function updateTask(taskDir: string): Promise<void> {
